...
frameworks.forEach((framework) => {
  test.describe(`${framework} a11y:${title}`, () => {
    ...
    test("axe check matrix", async ({ page }) => {
    ...
    test("axe check matrixdropdown", async ({ page }) => {
      const axeOptionsIgnoreMinor: RunOptions = {
        runOnly: {
          type: "tag",
          values: axeTags
        },
      ...
      await checkA11y(page, axeContext, { axeOptions: axeOptionsIgnoreMinor });
    });
    test("axe check matrixdynamic", async ({ page }) => {
      await initSurvey(page, framework, {
        "elements": [
          {
            type: "matrixdynamic",
    ...
    test("axe check matrixdropdown with radiogroup cell type", async ({ page }) => {
      await initSurvey(page, framework, {
        "elements": [
          {
            "type": "matrixdropdown",
      ...
      await page.evaluate(() => {
        (window as any).survey.getQuestionByName("quality").visibleRows[0].showHideDetailPanelClick();
      });
      await checkA11y(page, axeContext, { axeOptions: axeOptions });
    });
    test("axe check matrixdropdown with checkbox cell type", async ({ page }) => {
      await initSurvey(page, framework, {
        "elements": [
          {
            "type": "matrixdropdown",
