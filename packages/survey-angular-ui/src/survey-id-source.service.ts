...
/**
 * App-scoped source of SSR-stable `postId` tokens for Angular (see v3-element-ids-02).
 *
 ...
 * automatic multi-survey-SSR path. While disabled, `next()` returns `""` and `postId` stays empty -
 * single-survey SSR and today's client behavior are unchanged.
 */
@Injectable({ providedIn: "root" })
export class SurveyIdSourceService {
  /**
   * Flip to `true` on a hydration-capable Angular to enable automatic per-survey `postId` tokens.
