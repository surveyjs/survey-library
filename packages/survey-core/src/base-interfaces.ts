...
export interface ISaveToJSONOptions {
   ...
   * Possible values:
   *
   * - `true` (default) &mdash; Export the full JSON schema, including all locale strings. Use the [`locales`](#locales) array to restrict the output to specific locales.
   * - `false` &mdash; Export the JSON schema without any textual content.
   * - `"stringsOnly"` &mdash; Export a JSON schema that contains only locale strings and the minimal set of properties required to identify survey elements. Use the [`locales`](#locales) array to restrict the output to specific locales.
   */
