...
export class Base implements IObjectValueContext {
   ...
   * DOM on its own, so it does not require per-survey/SSR determinism. It is sourced from the shared
   * fallback generator's counter rather than a `Base`-level mutable static field.
   */
  ...
  /**
   * A value assigned to the `id` attribute of the rendered HTML element. A deterministic default is
   * generated automatically on first access; assigning a value explicitly always wins.
   */
  ...
  public getIsSerializablePropertyEmpty(prop: JsonObjectProperty): boolean {
    ...
    if (prop.isLocalizable && !prop.isMultipleText) return !orgObj.getLocalizableString(prop.name);
