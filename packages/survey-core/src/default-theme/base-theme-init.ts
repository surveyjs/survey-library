...
/**
 * Injects BaseTheme CSS variables into the document or shadow root that owns `htmlElement`.
 * Called from `SurveyModel.afterRenderSurvey()` after the survey is mounted, so the
 * `.sd-theme-root` rules are reachable both in the main document and inside shadow DOM.
 */
