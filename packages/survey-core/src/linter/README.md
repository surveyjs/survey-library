...
| `name/reserved` | error | A name that is a member of `Object.prototype` — `constructor`, `__proto__`, `toString`, `valueOf`, `hasOwnProperty`, `isPrototypeOf`, `propertyIsEnumerable`, `toLocaleString`, `__defineGetter__`, `__defineSetter__`, `__lookupGetter__`, `__lookupSetter__` — as a question `name` or `valueName`, a column or a row of a matrix, an item of a multiple text, or a calculated value. The runtime keeps survey data in plain objects, so such a key answers the prototype member instead of the value: a question with one of these names does not even load, a column or an item throws on the first write, a row loses its value or reads as answered before it is, a calculated value named `__proto__` never reaches the result. The comparison trims and ignores case — `Constructor`, `TOSTRING` and `" toString "` are reserved too, since the runtime trims a name and lower-cases its variable names and its question lookup hashes — and the name is reported in every one of these places alike, even where a particular slot happens to survive today: a name that works in one place and breaks in another is the confusing part. Pages, panels and the columns of a single-choice matrix are not data keys and are left alone; so are the elements of a composite component definition, which are never indexed. |
