...
function respellRaw(ref: ParsedRef): string | undefined {
  if (!ref.suggestion || !ref.raw) return undefined;
  const parts = ref.raw.split(".");
  const index = ref.unknownSegmentIndex || 0;
  if (index >= parts.length) return undefined;
  parts[index] = ref.suggestion;
  const res = parts.join(".");
  return res === ref.raw ? undefined : res;
}
...
function buildRefFix(path: string, value: string): ILintFix | undefined {
  if (value === undefined) return undefined;
  return { reason: fixReasons.renameReference, edits: [{ op: "set", path: path, value: value }] };
}
  ...
  run(ctx: LintContext): void {
    ctx.forEachSite("parsed", site => {
      classifySiteRefs(site, ctx.index, ctx.options).forEach(ref => {
        ...
        reportRef(ctx, ref, {
          ...
          // an inArray filter and a condition synthesized from a legacy trigger are strings the
          // analysis carved out, and the document has no property holding them
          fix: !!site.inArrayOf || !!site.synthesized ? undefined
            : buildRefFix(site.path, rewriteReference(site.text, ref, ctx.index.settings)),
        });
      });
    ...
    ctx.index.nameRefs.forEach((nameRef: NameRef) => {
      ...
      reportRef(ctx, ref, {
        ...
        fix: buildRefFix(nameRef.path, nameRef.kind === "binding"
          ? respellRaw(ref)
