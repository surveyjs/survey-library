...
describe("Expressions", () => {
  test("Logic Operand", () => {
    ...
    expect(expression.toString()).toBe("(((((false or true) or false) or false) or true) or true)");
    expect(expression.evaluate()).toBe(true);
  });
  ...
  test("Comparable Operand", () => {
    ...
    expect(expression.toString()).toBe("(false or (1 > -101))");
    expect(expression.evaluate()).toBe(true);

    expression = parse("true && 5 <= 12 || !(5 > 2)");
    expect(expression.toString()).toBe("((true and (5 <= 12)) or ! (5 > 2))");
    expect(expression.evaluate()).toBe(true);
  });
  ...
  test("Arithmetic Operand", () => {
    ...
    expect(expression.toString()).toBe("(((2 + 2) < (3 + 3)) and (5 < 10))");
    expect(expression.evaluate()).toBe(true);
  });

  test("Concat strings", () => {
    var expression = parse("'ab' + 'cd'");
    expect(expression.evaluate()).toBe("abcd");
    expression = parse("'ab' + ' ' + 'cd'");
    expect(expression.evaluate()).toBe("ab cd");
    expression = parse("1 + 'a' + 2");
    expect(expression.evaluate()).toBe("1a2");
    expression = parse("1 + ' ' + 2");
    expect(expression.evaluate()).toBe("1 2");
    expression = parse('"p1. " + "p2"');
    expect(expression.evaluate()).toBe("p1. p2");
  });

  test("Variable Const", () => {
    var varOperand = parse("'Im-Variable'");
    expect(varOperand.toString()).toBe("'Im-Variable'");
    expect(varOperand.evaluate()).toBe("Im-Variable");

    var constOperand = parse("{ImConst}");
    expect(constOperand.toString()).toBe("{ImConst}");

    var processValue = new ProcessValue(new VariableGetterContext({ ImConst: 7 }));
    expect(constOperand.evaluate(processValue)).toBe(7);
  });
  ...
  test("[] notcontains 0 returns false #10869", () => {
    ...
    expect(runner.runValues({ a: [] }), "[] notcontains 0").toBe(true);
    expect(runner.runValues({ a: [1, 2] }), "[1,2] notcontains 0").toBe(true);
    expect(runner.runValues({ a: [0, 1] }), "[0,1] notcontains 0").toBe(false);
  ...
  test("Run one condition", () => {
    ...
    var values = { a: 6 };
    expect(runner.runValues(values), "6 > 5").toBe(true);
    values = { a: 5 };
    expect(runner.runValues(values), "5 > 5").toBe(false);
    var values2 = { b: 5 };
    expect(runner.runValues(values2), "undefined > 5").toBe(false);
  });
  ...
  test("Run complex condition", () => {
    ...
    expect(runner.runValues(values), "21 >= 21 and (male = male or 1 > 1)").toBe(true);
    var values = { age: 21, sex: "female", kids: 1 };
    expect(runner.runValues(values), "21 >= 21 and (male = female or 1 > 1)").toBe(false);
    var values = { age: 21, sex: "female", kids: 2 };
    expect(runner.runValues(values), "21 >= 21 and (male = female or 2 > 1)").toBe(true);
    var values = { age: 20, sex: "male", kids: 2 };
    expect(runner.runValues(values), "20 >= 21 and (male = male or 2 > 1)").toBe(false);
  });
  ...
  test("Run condition with nested properties", () => {
    ...
    expect(runner.runValues(values), "min > 35 max < 80").toBe(true);
    values.age.min = 21;
    expect(runner.runValues(values), "min < 35 max < 80").toBe(false);
  });
  ...
  test("Condition check #303", () => {
    ...
    var values = {};
    expect(runner.runValues(values), "nothing was set").toBe(false);
    values = {
      "question-fruit": "fruit-apple",
      "question-apple-variety": "apple-variety-red-delicious",
    };
    expect(runner.runValues(values), "The first part is correct").toBe(true);
    values["question-fruit"] = "fruit-orange";
    expect(runner.runValues(values), "the value is incorrect").toBe(false);
    values["question-orange-variety"] = "orange-variety-blood";
    expect(runner.runValues(values), "The second part is correct").toBe(true);
  });

  test("Condition check empty for undefined variables #323", () => {
    var runner = new ConditionRunner("{var1} empty");
    var values = {};
    expect(runner.runValues(values), "it is empty").toBe(true);
    values = { var1: 1 };
    expect(runner.runValues(values), "it is not empty").toBe(false);
  });

  test("Condition check for undefined variables #323", () => {
    var runner = new ConditionRunner("{var1} < 3 or {var1} empty");
    var values = {};
    expect(runner.runValues(values), "empty should work").toBe(true);
    values = { var1: 1 };
    expect(runner.runValues(values), "1 < 3").toBe(true);
    values = { var1: 5 };
    expect(runner.runValues(values), "5 > 3").toBe(false);
  });

  test("Check non equal, #377", () => {
    var runner = new ConditionRunner("{var1} != 3");
    var values = {};
    expect(runner.runValues(values), "empty should give true").toBe(true);
    values = { var1: 1 };
    expect(runner.runValues(values), "1 != 3").toBe(true);
    values = { var1: 3 };
    expect(runner.runValues(values), "3 == 3").toBe(false);
  });

  test("Condition check for undefined #518", () => {
    var runner = new ConditionRunner("{var1} == undefined");
    var values = {};
    expect(runner.runValues(values), "undefined should work").toBe(true);
    values = { var1: undefined };
    expect(runner.runValues(values), "undefined should work").toBe(true);
    values = { var1: "a" };
    expect(runner.runValues(values), "string is not undefined").toBe(false);

    runner = new ConditionRunner("{var1} != undefined");
    values = {};
    expect(runner.runValues(values), "undefined should work").toBe(false);
    values = { var1: undefined };
    expect(runner.runValues(values), "undefined should work").toBe(false);
    values = { var1: "a" };
    expect(runner.runValues(values), "string is not undefined").toBe(true);
  });
  test("Run count function with arrays", () => {
    var runner = new ExpressionRunner("count({var1})");
    var values = { var1: [2, 5], var2: 3 };
    expect(runner.runValues(values), "two rows").toBe(2);
  });

  test("Run sum function", () => {
    var runner = new ExpressionRunner("sum({var1},{var2},{var3},{var4})");
    var values = { var1: 2, var2: 3, var3: 4, var4: 5 };
    expect(runner.runValues(values), "2 + 3 + 4 + 5 == 14").toBe(14);
    values.var1 = 1;
    expect(runner.runValues(values), "1 + 3 + 4 + 5 == 13").toBe(13);
  });

  test("Run sum function with arrays, Bug #1808", () => {
    var runner = new ExpressionRunner("sum({var1},{var2})");
    var values = { var1: [2, 5], var2: 3 };
    expect(runner.runValues(values), "2 + 5 + 3 == 10").toBe(10);
  });
  ...
  test("Run round() function with precision", () => {
    ...
    expect(runner.runValues(values), "1.005 is rounded to 1.01").toBe(1.01);
    values = { num: 2.175, precision: 2 };
    expect(runner.runValues(values), "2.175 is rounded to 2.18").toBe(2.18);
    values = { num: 5.015, precision: 2 };
    expect(runner.runValues(values), "5.015 is rounded to 5.02").toBe(5.02);
  ...
  test("Run trunc() function without precision", () => {
    ...
    var values = { num: 1.5 };
    expect(runner.runValues(values), "1.5 is truncated to 1").toBe(1);
    values = { num: -1.5 };
    expect(runner.runValues(values), "-1.5 is truncated to -1").toBe(-1);
    values = { num: -0.5 };
    expect(runner.runValues(values), "-0.5 is truncated to 0").toBe(-0);
    (<any>values).num = "-0.5";
    expect(runner.runValues(values), "A string value \"-0.5\" is truncated to a numeric value 0").toBe(-0);
    (<any>values).num = undefined;
    expect(Number.isNaN(runner.runValues(values)), "The value passed to the trunc() function is not a number").toBe(true);
  });
  test("Run trunc() function with precision", () => {
    const runner = new ExpressionRunner("trunc({num}, {precision})");
    var values = { num: 1.005, precision: 2 };
    expect(runner.runValues(values), "1.005 is truncated to 1 with precision of 2").toBe(1);
    values = { num: 2.175, precision: 1 };
    expect(runner.runValues(values), "2.175 is truncated to 2.1 with precision of 1").toBe(2.1);
    values = { num: 5.015, precision: 2 };
  ...
  test("Run age function", () => {
    ...
    expect(runner.runValues(values), "true, bithday of 1974 >= 21").toBe(true);
    var curDate = new Date(Date.now());
    values.bithday = new Date(curDate.getFullYear() - 10, 1, 1);
    expect(runner.runValues(values), "false, the person is 10 years old").toBe(false);
  });
  test("Run age function, Bug#2562", () => {
    ...
    expect(runner.runValues(values), "80 years old, bithday is today").toBe(80);
    date = new Date(date.getTime() + 60 * 60 * 24 * 1000);
    values.birthday = date;
    expect(runner.runValues(values), "one day till 80").toBe(79);
  });
  test("Run dateDiff by years function", () => {
    ...
    expect(runner.runValues(values), "Use the second parameter").toBe(40);
  });
  test("Run age by months function", () => {
    ...
    expect(runner.runValues(values), "10 years old, bithday is today").toBe(10 * 12);
    date = new Date(date.getTime() + 60 * 60 * 24 * 1000);
    values.birthday = date;
    expect(runner.runValues(values), "9 years + 11 months").toBe(9 * 12 + 11);
  });
  test("Run dateDiff by months", () => {
    ...
    expect(runner.runValues(values), "Use the second parameter, #1").toBe(2 * 12 + 1);
    values.currentDate = new Date(2014, 11, 9);
    expect(runner.runValues(values), "Use the second parameter, #2").toBe(2 * 12);
  });
  test("Run dateDiff by days", () => {
    ...
    expect(runner.runValues(values), "32 days").toBe(32);
  ...
  test("Run dateDiff by hours", () => {
    ...
    const values = { d1: d1, d2: d2 };
    expect(runner.runValues(values), "35 hours").toBe(24 + 11);
    (<any>values).d1 = undefined;
    expect(runner.runValues(values), "a value is undefined").toBeNull();
  });
  test("Run dateDiff by minutes", () => {
    ...
    const values = { d1: d1, d2: d2 };
    expect(runner.runValues(values), "hours").toBe((24 + 11) * 60 + 15);
    (<any>values).d1 = undefined;
    expect(runner.runValues(values), "a value is undefined").toBeNull();
  });
  test("Run dateDiff by minutes Bug#10177", () => {
    ...
    const values = { d1: d1, d2: d2 };
    expect(runner.runValues(values), "minutes").toBe(5);
  });
  test("Run dateDiff by seconds #10176", () => {
    ...
    const values = { d1: d1, d2: d2 };
    expect(runner.runValues(values), "seconds").toBe(77);
  });
  test("Run dateDiff by days across DST boundary Bug#11029", () => {
    ...
    expect(runner.runValues(values), "days between 2026-03-15 and 2027-03-14").toBe(364);
    values.d2 = "2027-03-15";
    expect(runner.runValues(values), "days between 2026-03-15 and 2027-03-15").toBe(365);
  ...
  test("Run getYear() function", () => {
    ...
    var values = { birthday: new Date(1974, 1, 1) };
    expect(runner.runValues(values)).toBe(1974);
  });

  test("Run currentYear() function", () => {
    var runner = new ExpressionRunner("currentYear()");
    expect(runner.runValues({})).toBe(new Date().getFullYear());
  });
  ...
  test("Run today function", () => {
    ...
    expect(getDateStr(runVal), "check today").toBe(todayStr);
  });
  ...
  test("Compare date with today", () => {
    ...
    expect(condition.runValues(val), "yesterday < today").toBe(true);
    val.d = todayDate;
    expect(condition.runValues(val), "todayDate < today").toBe(false);
    expect(condition.runValues(val), "tomorrow < today").toBe(false);

    condition = new ConditionRunner("{d} <= today()");
    val.d = yesterday;
    expect(condition.runValues(val), "yesterday <= today").toBe(true);
    val.d = todayDate;
    expect(condition.runValues(val), "todayDate <= today").toBe(true);
    val.d = tomorrow;
    expect(condition.runValues(val), "tomorrow <= today").toBe(false);

    condition = new ConditionRunner("{d} >= today()");
    val.d = yesterday;
    expect(condition.runValues(val), "yesterday >= today").toBe(false);
    val.d = todayDate;
    expect(condition.runValues(val), "todayDate >= today").toBe(true);
    val.d = tomorrow;
    expect(condition.runValues(val), "tomorrow >= today").toBe(true);
  ...
  test("Run age function with empty value", () => {
    ...
    var values = {};
    expect(runner.runValues(values), "1. false, bithday is empty").toBe(false);
    expect(runner2.runValues(values), "2. false, bithday is empty").toBe(false);
  });
  ...
  test("Run function with properties", () => {
    ...
    expect(runner.runValues(values, properties), "3 = 3").toBe(true);
    properties.propValue = 5;
    expect(runner.runValues(values, properties), "3 != 5").toBe(false);
  ...
  test("Support true/false constants, #643", () => {
    ...
    var values = { year: 22 };
    expect(runner.runValues(values), "true, true && 22 >= 21").toBe(true);
    values = { year: 20 };
    expect(runner.runValues(values), "false, true && 20 >= 21").toBe(false);

    runner = new ConditionRunner("true or {year} >= 21");
    values = { year: 22 };
    expect(runner.runValues(values), "true, true or 22 >= 21").toBe(true);
    values = { year: 20 };
    expect(runner.runValues(values), "true, true or 20 >= 21").toBe(true);

    runner = new ConditionRunner("false && {year} >= 21");
    values = { year: 22 };
    expect(runner.runValues(values), "false, false && 22 >= 21").toBe(false);
    values = { year: 20 };
    expect(runner.runValues(values), "false, false && 20 >= 21").toBe(false);

    runner = new ConditionRunner("false or {year} >= 21");
    values = { year: 22 };
    expect(runner.runValues(values), "true, false or 22 >= 21").toBe(true);
    values = { year: 20 };
    expect(runner.runValues(values), "false, false or 20 >= 21").toBe(false);
  });

  test("true/false as string, bug#729", () => {
    var runner = new ConditionRunner("{isTrue} = 'true'");
    var values = { isTrue: "true" };
    expect(runner.runValues(values), "true, 'true' = 'true'").toBe(true);
  });

  test("true/false as constant in the left", () => {
    var runner = new ConditionRunner("true = {isTrue}");
    var values = { isTrue: "false" };
    expect(runner.runValues(values), "false, true = false").toBe(false);
  });

  test("Constant string with dash (-) doens't work correctly", () => {
    var runner = new ConditionRunner("{a} = '01-01-2018'");
    var values = { a: "01-01" };
    expect(runner.runValues(values), "'01-01' = '01-01-2018'").toBe(false);
    values.a = "01-01-2018";
    expect(runner.runValues(values), "'01-01-2018' = '01-01-2018'").toBe(true);
  });

  test("Bug with contains, bug#781", () => {
    var runner = new ConditionRunner("{ResultaatSelectie} contains '1'");
    var values = { ResultaatSelectie: ["1"] };
    expect(runner.runValues(values), "['1'] contains '1'").toBe(true);
    values = { ResultaatSelectie: ["2"] };
    expect(runner.runValues(values), "['2'] contains '1'").toBe(false);
  });

  test("Check contains with empty array, Bug #2193", () => {
    var runner = new ConditionRunner("[] contains 'C1'");
    expect(runner.runValues({}), "[] doesn't contain 'C1'").toBe(false);
  ...
  test("Bug with contains, support string.indexof, bug#831", () => {
    ...
    expect(runner.runValues(values), "'12345' contains '1'").toBe(true);
    values = { str: "2345" };
    expect(runner.runValues(values), "'2345' contains '1'").toBe(false);
    runner = new ConditionRunner("{str} notcontains '1'");
    expect(runner.runValues(values), "'2345' notcontains '1'").toBe(true);
    values = { str: "12345" };
    expect(runner.runValues(values), "'12345' notcontains '1'").toBe(false);
  });

  test("Bug with contains, bug#1039", () => {
    var runner = new ConditionRunner("{ValueType} contains '3b'");
    var values = { ValueType: ["3b"] };
    expect(runner.runValues(values), "['3b'] contains '3b'").toBe(true);
    values = { ValueType: ["1"] };
    expect(runner.runValues(values), "['1'] contains '3b'").toBe(false);
  });
  ...
  test("Add support for array for cotains operator, issue#1366", () => {
    ...
    expect(runner.runValues(values), "['a', 'b'] contains ['a', 'b']").toBe(true);
    values = { value: ["a", "c"] };
    expect(runner.runValues(values), "['a', 'c'] contains ['a', 'b']").toBe(false);
    values = { value: ["a", "b", "c"] };
    expect(runner.runValues(values), "['a', 'b', 'c'] contains ['a', 'b']").toBe(true);
  });

  test("Escape quotes, bug#786", () => {
    var runner = new ConditionRunner("{text} = 'I\\'m here'");
    var values = { text: "I'm here" };
    expect(runner.runValues(values), "text equals I'm here").toBe(true);

    var runner = new ConditionRunner(
      "'I said: \\\"I\\'m here\\\"' contains {text}"
    );
    var values = { text: "I'm here" };
    expect(runner.runValues(values), "I said contains text").toBe(true);
  });

  test("Support equals and notequals, #781", () => {
    var runner = new ConditionRunner("{a} equals 1");
    var values = { a: 1 };
    expect(runner.runValues(values), "1 equals 1").toBe(true);
    values = { a: 2 };
    expect(runner.runValues(values), "2 equals 1").toBe(false);
  });
  test("Allow differnt symbols in variable name, bug#803", () => {
    ...
    expect(runner.runValues(values), "1= 1").toBe(true);
    values = { "complex name #$%?dd": 2 };
    expect(runner.runValues(values), "2 <> 1").toBe(false);
  });

  test("Support array", () => {
    var runner = new ConditionRunner("{a} equals [1, 2]");
    var values = { a: [1, 2] };
    expect(runner.runValues(values), "[1, 2] equals [1, 2]").toBe(true);
    values = { a: [2] };
    expect(runner.runValues(values), "[2] equals [1, 2]").toBe(false);
    values = { a: [2, 1] };
    expect(runner.runValues(values), "[2, 1] equals [1, 2]").toBe(true);
  });
  ...
  test("ExpressionOperand: brackets", () => {
    ...
    var values = { a: 1, b: 3 };
    expect(runner.runValues(values), "(1 + 3) * 2 >= 10").toBe(false);
  });

  test("ExpressionOperand: brackets 2", () => {
    var runner = new ConditionRunner("({a} + {b} + {c}) / 3 >= 3");
    var values = { a: 1, b: 3, c: 2 };
    expect(runner.runValues(values), "(1 + 3 + 2) / 3 >= 3").toBe(false);
    values.c = 5;
    expect(runner.runValues(values), "(1 + 3 + 4) / 3 >= 3").toBe(true);
  });

  test("ConditionRunner: (1+2)*3", () => {
    var runner = new ExpressionRunner("(1+2)*3");
    expect(runner.runValues({}), "(1+2)*3 is 9").toBe(9);
  });

  test("ConditionRunner: (1+2)*(3+2) / 5", () => {
    var runner = new ExpressionRunner("(1+2)*(3+2) / 5");
    expect(runner.runValues({}), "(1+2)*(3+2) / 5 is 3").toBe(3);
  });

  test("ConditionRunner: 10 % 3", () => {
    var runner = new ExpressionRunner("10 % 3");
    expect(runner.runValues({}), "10 % 3 is 1").toBe(1);
    var condition = new ConditionRunner("({val1} + {val2}) % 2 = 0");
    expect(condition.runValues({ val1: 1, val2: 3 }), "(1+3)%2=0").toBe(true);
    expect(condition.runValues({ val1: 2, val2: 3 }), "(2+3)%2=0").toBe(false);
  });

  test("ExpressionRunner: sumInArray", () => {
    var runner = new ExpressionRunner("sumInArray({a}, 'val1')");
    var values = { a: [{ val1: 10 }, { val2: 10 }, { val1: 20 }] };
    expect(runner.runValues(values), "10 + 20").toBe(30);
    values = { a: [{ val2: 1 }] };
    expect(runner.runValues(values), "There is no values").toBe(0);
  });
  ...
  test("ExpressionRunner: sumInArray with conditional logic", () => {
    ...
    expect(runner.runValues(values), "1 + 3").toBe(4);
  });
  ...
  test("ExpressionRunner: countInArray with conditional logic & strings & for object value, #1", () => {
    ...
    var values = { a: { row1: { val1: 1, val2: "item2" }, row2: { val1: 2, val2: "item1" }, row3: { val1: 3, val2: "item3" } } };
    expect(runner.runValues(values), "1 & 3").toBe(2);
  });
  test("ExpressionRunner: countInArray with conditional logic & strings & for object value, #2", () => {
    ...
    expect(runner.runValues(values), "1 & 2").toBe(2);
  });
  test("ExpressionRunner: UnaryOperand, Bug#10068", () => {
    const operandNotEmpty = parse("{a} notempty");
    expect(operandNotEmpty.getType(), "Operand is unary, #1").toBe("unary");
    expect(operandNotEmpty.toString(), "Operand expression is correct, #1").toBe("{a} notempty");
    const operandEmpty = parse("{a} empty");
    expect(operandEmpty.getType(), "Operand is unary, #2").toBe("unary");
    expect(operandEmpty.toString(), "Operand expression is correct, #2").toBe("{a} empty");
  });
  test("ExpressionRunner: countInArray", () => {
    var runner = new ExpressionRunner("countInArray({a}, 'val1')");
    var values = { a: [{ val1: 10 }, { val2: 10 }, { val1: 20 }] };
    expect(runner.runValues(values), "10 + 20").toBe(2);
    values = { a: [{ val2: 1 }] };
    expect(runner.runValues(values), "There is no values").toBe(0);
    var emptyValue = { a: {} };
    expect(runner.runValues(emptyValue), "object is empty").toBe(0);
  });
  test("ExpressionRunner: countInArray, value as string", () => {
    var runner = new ExpressionRunner("countInArray({a}, 'val1')");
    var values = { a: [{ val1: "abc" }, { val2: 10 }, { val1: "cde" }] };
    expect(runner.runValues(values), "two items in array").toBe(2);
  });

  test("ConditionRunner, iif simple", () => {
    var runner = new ExpressionRunner("iif({a}, 'high', 'low')");
    var values = { a: true };
    expect(runner.runValues(values), "true").toBe("high");
    values.a = false;
    expect(runner.runValues(values), "false").toBe("low");
  });

  test("ExpressionRunner, iif with expression", () => {
    var runner = new ExpressionRunner("iif({a} + {b} > 20, 'high', 'low')");
    var values = { a: 10, b: 20 };
    expect(runner.runValues(values), "10 + 20 > 20").toBe("high");
    values.b = 5;
    expect(runner.runValues(values), "10 + 5 < 20").toBe("low");
  });
  ...
  test("ConditionRunner, iif nested using", () => {
    ...
    expect(runner.runValues(values), "10 + 20 > 20").toBe("high");
    values.b = 5;
    expect(runner.runValues(values), "10 + 5 > 10 && 10 + 5 < 20").toBe("medium");
    values.a = 1;
    expect(runner.runValues(values), "1 + 5 < 10").toBe("low");
  });
  ...
  test("ConditionRunner, iif nested using 2", () => {
    ...
    var values = { a: 10, b: 20 };
    expect(runner.runValues(values), "10 + 20 > 20").toBe(10 * 5 + 20);
    values.b = 5;
    expect(runner.runValues(values), "10 + 5 > 10 && 10 + 5 < 20").toBe(5 * (10 + 5));
    values.a = 1;
    expect(runner.runValues(values), "1 + 5 < 10").toBe(1);
  });

  test("ConditionRunner, iif with empty parameter, Bug#2732", () => {
    var runner = new ExpressionRunner("iif({a}, 'yes', '')");
    var values = { a: true };
    expect(runner.runValues(values), "true").toBe("yes");
    values.a = false;
    expect(runner.runValues(values), "false").toBe("");
  });
  ...
  test("ConditionRunner, iif nested using with function, Bug T1302, (https://surveyjs.answerdesk.io/ticket/details/T1302)", () => {
    ...
    expect(runner.runValues(values), "the first condition is calling").toBe(1 + 2 + 3 + 1);
  ...
  test("ConditionRunner, ^ operator", () => {
    ...
    expect(runner.runValues(values), "10^3 + 400^0.5 = 1000 + 20").toBe(1020);
  });
  ...
  test("Variable equals 0x1 works incorrectly, Bug#1180", () => {
    ...
    var values = { val: "0x1" };
    expect(runner.runValues(values), "0x1 is not empty").toBe(true);

    runner = new ConditionRunner("{val} = 2");
    values = { val: "0x1" };
    expect(runner.runValues(values), "0x1 is not 2").toBe(false);
    values = { val: "0x2" };
    expect(runner.runValues(values), "0x2 is not 2").toBe(true);
  });

  test("notempty with 0 and false, bug#1792", () => {
    var runner = new ConditionRunner("{val} notempty");
    var values: any = { val: 0 };
    expect(runner.runValues(values), "0 is not empty").toBe(true);
    values.val = "0";
    expect(runner.runValues(values), "'0' is not empty").toBe(true);
    values.val = false;
    expect(runner.runValues(values), "false is not empty").toBe(true);
  });

  test("contain and noncontain for null arrays", () => {
    var runner = new ConditionRunner("{val} contain 1");
    var values = {};
    expect(runner.runValues(values), "undefined doesn't contain 1 - false").toBe(false);
    runner = new ConditionRunner("{val} notcontain 1");
    values = {};
    expect(runner.runValues(values), "undefined doesn't contain 1 - true").toBe(true);
  });

  test("length for defined arrays", () => {
    var runner = new ConditionRunner("{val.length} > 1");
    expect(runner.runValues({ val: [] }), "empty array length returns 0").toBe(false);
    expect(runner.runValues({ val: [1] }), "array length returns 1").toBe(false);
    expect(runner.runValues({ val: [1, 2] }), "array length returns 2").toBe(true);
    expect(runner.runValues({ val: [1, 2, 3] }), "array length returns 3").toBe(true);
  });

  test("length for undefined arrays", () => {
    var runner = new ConditionRunner("{val.length} = 0");
    expect(runner.runValues({ val: [] }), "empty array length returns 0").toBe(true);
    expect(runner.runValues({}), "undefined length returns 0").toBe(true);
    expect(runner.runValues({ val: undefined }), "undefined length returns 0").toBe(true);
    expect(runner.runValues({ val: null }), "null length returns 0").toBe(true);
    runner = new ConditionRunner("{val.length} < 4");
    expect(runner.runValues({ val: [] }), "empty array length < 4").toBe(true);
    expect(runner.runValues({}), "undefined length < 4").toBe(true);
    expect(runner.runValues({ val: undefined }), "undefined length  < 4").toBe(true);
    expect(runner.runValues({ val: null }), "null length  < 4").toBe(true);
  });
  ...
  test("length for arrays that becomes undefined, Bug#2432", () => {
    ...
    expect(runner.runValues(data), "[1,2].length < 3").toBe(true);
    data.val = undefined;
    expect(runner.runValues(data), "undefined.length < 3").toBe(true);
  });

  test("contain and noncontain for strings", () => {
    var runner = new ConditionRunner("{val} contain 'ab'");
    var values = {};
    expect(runner.runValues(values), "contains: undefined doesn't contain 'ab' - false").toBe(false);
    values = { val: "ba" };
    expect(runner.runValues(values), "contains: 'ba' doesn't contain 'ab' - false").toBe(false);
    values = { val: "babc" };
    expect(runner.runValues(values), "contains: 'babc' contains 'ab' - true").toBe(true);

    runner = new ConditionRunner("{val} notcontain 'ab'");
    values = {};
    expect(runner.runValues(values), "notcontains: undefined doesn't contain 'ab' - true").toBe(true);
    values = { val: "ba" };
    expect(runner.runValues(values), "notcontains: 'ba' doesn't contain 'ab' - true").toBe(true);
    values = { val: "babc" };
    expect(runner.runValues(values), "notcontains: 'babc' contains 'ab' - false").toBe(false);
  });

  test("ConditionRunner: 7 * (({q1} * 0.4) + ({q2} * 0.6)), bug# 1423", () => {
    var runner = new ExpressionRunner("7 * ((10 * 0.4) + (20 * 0.6))");
    expect(runner.runValues({}), "7 * ((10 * 0.4) + (20 * 0.6)) is 112").toBe(7 * (4 + 12));
  });

  test("0x digits", () => {
    var runner = new ExpressionRunner("0x1 + 0x2 + {x}");
    var values = { x: 0x3 };
    expect(runner.runValues(values), "0x: 1 + 2 + 3 equal 6").toBe(6);
  });
  ...
  test("Get variables in expression", () => {
    ...
    expect(vars.length, "There are 6 variables in expression").toBe(6);
    expect(vars[0], "the first variable").toBe("val1");
    expect(vars[5], "the last variable").toBe("val6");
  });
  test("Support negative index in arrays, Issue#10607", () => {
    ...
    expect(runner.runValues(values), "50 + 30 + 10 = 90").toBe(50 + 30 + 10);
  });
  test("Test binary operator anyof", () => {
    var runner = new ConditionRunner("{value} anyof ['a', 'b']");
    var values = { value: ["a", "c"] };
    expect(runner.runValues(values), "['a', 'c'] anyof ['a', 'b']").toBe(true);
    values = { value: ["a", "b"] };
    expect(runner.runValues(values), "['a', 'b'] anyof ['a', 'b']").toBe(true);
    values = { value: ["c", "d"] };
    expect(runner.runValues(values), "['c', 'd'] anyof ['a', 'b']").toBe(false);
    values = { value: [] };
    expect(runner.runValues(values), "[] anyof ['a', 'b']").toBe(false);
    values = { value: null };
    expect(runner.runValues(values), "null anyof ['a', 'b']").toBe(false);
  });
  test("Test binary operator noneof", () => {
    var runner = new ConditionRunner("{value} noneof ['a', 'b']");
    var values = { value: ["a", "c"] };
    expect(runner.runValues(values), "['a', 'c'] noneof ['a', 'b']").toBe(false);
    values = { value: ["a", "b"] };
    expect(runner.runValues(values), "['a', 'b'] noneof ['a', 'b']").toBe(false);
    values = { value: ["c", "d"] };
    expect(runner.runValues(values), "['c', 'd'] noneof ['a', 'b']").toBe(true);
    values = { value: [] };
    expect(runner.runValues(values), "[] noneof ['a', 'b']").toBe(true);
    values = { value: null };
    expect(runner.runValues(values), "null noneof ['a', 'b']").toBe(true);
  });
  test("Test binary operator anyof with 0", () => {
    var runner = new ConditionRunner("{value} anyof [7, 3, 0]");
    var values = { value: 3 };
    expect(runner.runValues(values), "3 anyof [7, 3, 0]").toBe(true);
    values.value = 0;
    expect(runner.runValues(values), "0 anyof [7, 3, 0]").toBe(true);
    values.value = 1;
    expect(runner.runValues(values), "1 anyof [7, 3, 0]").toBe(false);
  });
  test("Test operator anyof for non-array var", () => {
    var runner = new ConditionRunner("{value} anyof ['a', 'b', 'c']");
    var values = { value: "a" };
    expect(runner.runValues(values), "'a' anyof ['a', 'b', 'c']").toBe(true);
    values.value = "e";
    expect(runner.runValues(values), "'e' anyof ['a', 'b', 'c']").toBe(false);
  });
  test("Test binary operator allof", () => {
    var runner = new ConditionRunner("{value} allof ['a', 'b']");
    var values = { value: ["a", "c"] };
    expect(runner.runValues(values), "['a', 'c'] allof ['a', 'b']").toBe(false);
    values = { value: ["a", "b", "c"] };
    expect(runner.runValues(values), "['a', 'b', 'c'] allof ['a', 'b']").toBe(true);
    values = { value: ["c", "d"] };
    expect(runner.runValues(values), "['c', 'd'] allof ['a', 'b']").toBe(false);
  });
  ...
  test("Compare undefined object with string", () => {
    ...
    var values: any = {};
    expect(runner.runValues(values), "undefined = 'undefined'").toBe(true);
  });

  test("Compare two undefined variables", () => {
    var values: any = { v1: undefined, v2: undefined };
    expect(new ConditionRunner("{v1} = {v2}").runValues(values), "undefined = undefined").toBe(true);
    expect(new ConditionRunner("{v1} != {v2}").runValues(values), "undefined != undefined").toBe(false);
    expect(new ConditionRunner("{v1} <= {v2}").runValues(values), "undefined <= undefined").toBe(true);
    expect(new ConditionRunner("{v1} >= {v2}").runValues(values), "undefined >= undefined").toBe(true);
    expect(new ConditionRunner("{v1} < {v2}").runValues(values), "undefined < undefined").toBe(false);
    expect(new ConditionRunner("{v1} > {v2}").runValues(values), "undefined > undefined").toBe(false);
    values.v1 = 1;
    expect(new ConditionRunner("{v1} > {v2}").runValues(values), "1 > undefined").toBe(false);
    expect(new ConditionRunner("{v1} < {v2}").runValues(values), "1 < undefined").toBe(false);
  });
  ...
  test("String as numbers", () => {
    ...
    expect(runner.runValues(values), "convert strings to numbers").toBe(20);
  });
  ...
  test("True/False strings do not work, Bug #https://surveyjs.answerdesk.io/ticket/details/T2425", () => {
    ...
    expect(runner.runValues(values), "'True' = 'False'").toBe(false);
    values.a = "True";
    expect(runner.runValues(values), "'True' = 'True'").toBe(true);
    values.a = false;
    expect(runner.runValues(values), "false = 'True'").toBe(false);
    values.a = true;
    expect(runner.runValues(values), "true = 'True'").toBe(true);

    runner = new ConditionRunner("{a} = 'False'");
    values.a = "False";
    expect(runner.runValues(values), "'False' = 'False'").toBe(true);
    values.a = "True";
    expect(runner.runValues(values), "'True' = 'False'").toBe(false);
    values.a = false;
    expect(runner.runValues(values), "'False' = false").toBe(true);
  ...
  test("Async function", () => {
    ...
    expect(runner.isAsync, "The condition is async").toBe(true);
    var values = { a: 3 };
    runner.runValues(values);
    expect(runnerResult, "3*3 = 15").toBe(false);
    values.a = 5;
    runner.runValues(values);
    expect(runnerResult, "5*3 = 15").toBe(true);
  ...
  test("Async function inside the sync function, Bug#8732", () => {
    ...
    expect(runner.canRun(), "The expression is valid").toBe(true);
    expect(runner.isAsync, "It is async").toBe(true);
    var runnerResult = null;
    runner.onRunComplete = function(result: any) {
      runnerResult = result;
    };
    var values = { a: 3 };
    runner.runValues(values);
    expect(runnerResult, "3*3 + 10").toBe(19);
    values.a = 5;
    runner.runValues(values);
    expect(runnerResult, "5*3 + 10").toBe(25);
    FunctionFactory.Instance.unregister("asyncFunc");
  ...
  " + syncFunc2({e}, syncFunc1({a}, syncFunc1({b}, asyncFunc2({c}, {d}))))";
    ...
    runner.runValues(values);
    expect(runnerResult, "#1").toBe(57 + 42);
  ...
  test("Use onRunComplete for sync functions", () => {
    ...
    expect(runner.isAsync, "The condition is sync").toBe(false);
    var values = { a: 3 };
    runner.runValues(values);
    expect(runnerResult, "3*3 = 15").toBe(false);
    values.a = 5;
  ...
  test("Several async functions in expression", () => {
    ...
    expect(runner.isAsync, "The condition is async").toBe(true);
    var values = { a: 3 };
    expect(idBefore, "idBefore #1").toBe(-1);
    expect(idAfter, "idAfter #1").toBe(-1);
    runner.runValues(values);
    expect(idBefore, "idBefore #2").not.toBe(-1);
    expect(idAfter, "idAfter #2").toBe(-1);
    expect(runnerResult, "It is not ready, async functions do not return anything").toBeNull();
    returnResult2(2);
    expect(runnerResult, "It is not ready, asyncfunc1 and asyncfunc3 functions do not return anything").toBeNull();
    expect(idBefore, "idBefore #3").not.toBe(-1);
    expect(idAfter, "idAfter #3").toBe(-1);
    returnResult1(7);
    expect(runnerResult, "It is not ready, asyncfunc3 function doesn't return anything").toBeNull();
    expect(idBefore, "idBefore #4").not.toBe(-1);
    expect(idAfter, "idAfter #4").toBe(-1);
    returnResult3(1);
    expect(runnerResult, "evulate successfull").toBe(true);
    expect(idBefore, "idBefore #5").not.toBe(-1);
    expect(idAfter, "idAfter #5").toBe(idBefore);
    FunctionFactory.Instance.unregister("asyncFunc1");
    FunctionFactory.Instance.unregister("asyncFunc2");
  ...
  test("isString function", () => {
    ...
    var properties = {};
    expect(runner.runValues(values, properties), "3 is Numeric").toBe(false);
    values.val = false;
    expect(runner.runValues(values, properties), "false is boolean").toBe(false);
    values.val = "abc";
    expect(runner.runValues(values, properties), "'abc' is string").toBe(true);
    values.val = "0x2323";
    expect(runner.runValues(values, properties), "'0x2323' is number").toBe(false);
    values.val = "0xbe0eb53f46cd790cd13851d5eff43d12404d33e8";
    expect(runner.runValues(values, properties), "'0xbe0eb53f46cd790cd13851d5eff43d12404d33e8' is string").toBe(true);
  ...
  // prettier-ignore
    ...
    expect(runner.runValues(values), "val1 + [val1]").toBe("1[1]");
    values.val2 = "item1";
    expect(runner.runValues(values), "1 + 'x'").toBe("1x");
    values.val1 = undefined;
    expect(runner.runValues(values), "undefined + 'x'").toBe("x");
    // prettier-ignore
    expression = "{val1} + \"x\"";
    var runner = new ExpressionRunner(expression);
    expect(runner.runValues(values), "undefined + 'x' without iif").toBe("x");
    expression = '{val1} + "[" + {val1} + "]"';
    var runner = new ExpressionRunner(expression);
    expect(runner.runValues(values), "undefined + '[' + undefined + ']' without iif").toBe("[]");
  });
  ...
  // prettier-ignore
    ...
    expect(runner.runValues(values), "{").toBe("1{ text}");

    expression = "{val1} + '{ text' + '}'";
    runner = new ExpressionRunner(expression);
    var values: any = { val1: "1" };
    expect(runner.runValues(values), "{ without escape").toBe("1{ text}");
  });
  test("Disable converting string to number, #2376", () => {
    var runner = new ExpressionRunner("{val1} + {val2}");
    var values: any = { val1: "1", val2: "102" };
    expect(runner.runValues(values), "Convert strings to numbers").toBe(103);

    runner = new ExpressionRunner("{#val1} + {#val2}");
    expect(runner.runValues(values), "do not convert the value").toBe("1102");
    let expr = new ConditionsParser().createCondition("{#val1} + {#val2}");
    expect(expr.toString(), "Do not loose '#'").toBe("({#val1} + {#val2})");
  });
  ...
  test("ExpressionRunner: age", () => {
    ...
    var values = { d: d };
    expect(runner.runValues(values), "10 year").toBe(10);
  /* TODO case stop worked on December 31. Do we need minus age?
  d = new Date();
  d.setDate(d.getDate() + 1);
  d.setFullYear(d.getFullYear() + 10);
  values.d = d;
  expect(runner.run(values), "Date in the future, 10 years").toBe(-10);
  */
  });
  ...
  test("ExpressionRunner: diffsDays", () => {
    ...
    expect(runner.runValues(values), "32 days").toBe(32);
    values.d1 = undefined;
    expect(runner.runValues(values), "a value is undefined").toBe(0);
  });
  test("ExpressionRunner: diffsDays with times", () => {
    ...
    var values = { d1: d1, d2: d2 };
    expect(runner.runValues(values), "1 day").toBe(1);
    values.d2 = new Date("2021-03-10T23:00:00");
    expect(runner.runValues(values), "0 days").toBe(0);
  });
  ...
  test("parse({val} == '000')", () => {
    ...
    expect(op.rightOperand.getType(), "variable type").toBe("const");
    expect((<Const>op.rightOperand).toString(), "value is string with quotes").toBe("'000'");
  ...
  test("000 == '000', '00' != '000', '0' != '000', 0 != '000'", () => {
    ...
    expect(runner.runValues(values), "000 == '000'").toBe(true);
    values.val = "00";
    expect(runner.runValues(values), "'00' != '000'").toBe(false);
    values.val = "0";
    expect(runner.runValues(values), "'0' != '000', '0' and '000' converted to number").toBe(true);
    values.val = 0;
    expect(runner.runValues(values), "0 != '000', convert '000' to number").toBe(true);
  });
  ...
  test("ExpressionRunner: fix incorrect JavaScript summary in avgInArray function", () => {
    ...
    var values = { a: [{ val1: 0.3 }, { val2: 10 }, { val1: 0.6 }] };
    expect(runner.runValues(values), "It works correctly, avgInArray func").toBe(0.45);
  });
  test("ExpressionRunner: sumInArray function, float and string", () => {
    var runner = new ExpressionRunner("sumInArray({a}, 'val1')");
    var values = { a: [{ val1: 0.3 }, { val1: "10" }, { val1: "0.6" }, { val1: "abc" }] };
    expect(runner.runValues(values), "It works correctly, sumInArray func").toBe(10.9);
  });
  test("ExpressionRunner: minInArray function, float and string", () => {
    ...
    expect(runner.runValues(values), "It works correctly, minInArray func").toBe(1);
  });
  test("ExpressionRunner: maxInArray function, float and string", () => {
    var runner = new ExpressionRunner("maxInArray({a}, 'val1')");
    var values = { a: [{ val1: -3 }, { val1: "-1" }, { val1: "abc" }] };
    expect(runner.runValues(values), "It works correctly, maxInArray func").toBe(-1);
  });
  test("ExpressionRunner: minInArray with return column", () => {
    ...
    expect(runner.runValues(values), "returns name from row with min price").toBe("A");
  });
  test("ExpressionRunner: maxInArray with return column", () => {
    var runner = new ExpressionRunner("maxInArray({a}, 'price', 'name')");
    var values = { a: [{ price: 30, name: "C" }, { price: 10, name: "A" }, { price: 20, name: "B" }] };
    expect(runner.runValues(values), "returns name from row with max price").toBe("C");
  });
  test("ExpressionRunner: minInArray with return column and condition", () => {
    ...
    expect(runner.runValues(values), "returns name from row with min price where active is true").toBe("A");
  });
  ...
  test("Operand.isEqual()", () => {
    ...
    expect(compareOperands("{val} = 1", "{val} = 2"), "#1").toBe(false);
    expect(compareOperands("{val} = 1", "{val} = 1")).toBe(true), "#2";
    expect(compareOperands("{val} = 1 and func({a}) < 3", "{val} = 1 and func({a}) < 3"), "#3").toBe(true);
    expect(compareOperands("{val} = 1 and func({a}) < 3", "{val} = 1 and func({b}) < 3"), "#4").toBe(false);
    expect(compareOperands("func({a}) < 3", "func({b}) < 3"), "#4.1").toBe(false);
    expect(compareOperands("func({a, b, 5}) < 3", "func({a, b, 5}) < 3"), "#4.2").toBe(true);
    expect(compareOperands("{a} < 3", "{b} < 3"), "#4.3").toBe(false);
    expect(compareOperands("{val} = 1 and func({a}) < 3", "{val} = 1 or func({a}) < 3"), "#5").toBe(false);
    expect(compareOperands("{val} = 1", "{val} != 1"), "#6").toBe(false);
  });

  test("Expression decimal + string", () => {
    const expression = new ExpressionRunner("0.1 + 'abc'");
    expect(expression.runValues({})).toBe("0.1abc");
  });
  test("string contancts without brackets", () => {
    let expression = new ExpressionRunner("{a} = abc");
    expect(expression.runValues({ a: "abcd" }), "#1").toBe(false);
    expect(expression.runValues({ a: "abc" }), "#2").toBe(true);
    expression = new ExpressionRunner("{a} = ab_cd");
    expect(expression.runValues({ a: "abcd" }), "#3").toBe(false);
    expect(expression.runValues({ a: "ab_cd" }), "#4").toBe(true);
  });
  test("Expression string as ==", () => {
    const node = <Const>new ConditionsParser().parseExpression("'=='");
    expect(node.getType()).toBe("const");
    expect(node.correctValue).toBe("==");
    expect(new ExpressionRunner("'=='").runValues({})).toBe("==");
    expect(new ExpressionRunner("'aa' + '=='").runValues({})).toBe("aa==");
  });
  test("Arrays and plus operations", () => {
    ...
    expect(runner3.runValues(values3), "summary of numbers").toBe(10);
    const runner4 = new ExpressionRunner("{a} + ''");
    const values4 = { a: [1, 2, 3] };
    expect(runner4.runValues(values4), "summary of numbers").toBe("1, 2, 3");
  });
  ...
  test("Sum two float numbers as string", () => {
    ...
    expect(runner.runValues({ a: "1.1", b: "2.2" }), "#1").toBe(3.3);
    expect(runner.runValues({ a: "0.1", b: "0.2" }), "#2").toBe(0.3);
  });
  test("Warn in console if the expression is invalid", () => {
    ...
    expect(reportText).toBe("Invalid expression: '{a} ++'.");
    ...
    expect(reportText).toBeFalsy();
    runner.runValues({});
    expect(reportText).toBe("Unknown function name: 'tooday'.");
  ...
  test("ExpressionRunner: substring", () => {
    ...
    expect(runner.runValues(values), "abcdef").toBe("bc");
    values.s = undefined;
    expect(runner.runValues(values), "undefined").toBe("");
    values.s = 10;
    expect(runner.runValues(values), "10").toBe("");
  });
  test("ExpressionRunner: apply custom converter, #8634", () => {
    ...
    expect(runner.runValues(values), "apply custom convertr").toBe(110000);
  ...
  test("No params for iif function, Bug#9674", () => {
    ...
    const values: any = {};
    expect(runner.runValues(values), "Empty paramsters, #1").toBeNull();
    runner = new ExpressionRunner("iif('')");
    expect(runner.runValues(values), "Empty paramsters, #2").toBeNull();
  });
  ...
  test("Operand.addOperandsToList", () => {
    ...
    expect(list.length, "operands in the list #1").toBe(3);
    operand = new ConditionsParser().parseExpression("func({a}, 'd', 3) * 4 + {b} > 0");
    list = [];
    operand.addOperandsToList(list);
    expect(list.length, "operands in the list #1").toBe(11);
  });
  test("ExpressionRunner vs context", () => {
    const runner = new ExpressionRunner("{a} + {b}");
    expect(runner.runContext(new VariableGetterContext({ a: 1, b: 2 })), "#1").toBe(3);
  });
  test("Condition vs not boolean, Bug#10412", () => {
    ...
    expect(operand.toString(), "the expression is valid").toBe("(((age({a}) >= 18) and ! {b}) and ! {c})");
    const runner = new ExpressionRunner(exp);
    expect(runner.runContext(new VariableGetterContext({ a: "2000-01-01", b: false, c: false })), "#1").toBe(true);
    expect(runner.runContext(new VariableGetterContext({ a: "2000-01-01", b: true, c: false })), "#2").toBe(false);
    expect(runner.runContext(new VariableGetterContext({ a: "2000-01-01", b: "false", c: false })), "#3").toBe(true);
    expect(runner.runContext(new VariableGetterContext({ a: "2000-01-01", b: "true", c: false })), "#4").toBe(false);
    expect(runner.runContext(new VariableGetterContext({ a: "2000-01-01", b: true, c: true })), "#5").toBe(false);

    expect(runner.runContext(new VariableGetterContext({ a: "2020-01-01", b: false, c: false })), "#6").toBe(false);
    expect(runner.runContext(new VariableGetterContext({ a: "2020-01-01", b: true, c: false })), "#7").toBe(false);
    expect(runner.runContext(new VariableGetterContext({ a: "2020-01-01", b: "false", c: false })), "#8").toBe(false);
    expect(runner.runContext(new VariableGetterContext({ a: "2020-01-01", b: "true", c: false })), "#9").toBe(false);
    expect(runner.runContext(new VariableGetterContext({ a: "2020-01-01", b: true, c: true })), "#10").toBe(false);
  });
  test("Unary not !, Bug#10412", () => {
    const exp = "!{b} and !{c}";
    const operand = new ConditionsParser().parseExpression(exp);
    expect(operand.toString(), "the expression is valid").toBe("(! {b} and ! {c})");
    const runner = new ExpressionRunner(exp);
    expect(runner.runContext(new VariableGetterContext({ b: false, c: false })), "#1").toBe(true);
    expect(runner.runContext(new VariableGetterContext({ b: true, c: false })), "#3").toBe(false);
    expect(runner.runContext(new VariableGetterContext({ b: "false", c: false })), "#5").toBe(true);
    expect(runner.runContext(new VariableGetterContext({ b: "true", c: false })), "#6").toBe(false);
  });
  test("Work with objects like constants, Bug#10448", () => {
    ...
    expect(runner.runContext(new VariableGetterContext({})), "#3").toBe(false);
  });
});
