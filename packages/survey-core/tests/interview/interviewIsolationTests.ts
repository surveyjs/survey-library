...
// The interview ships as a separate bundle, and that only stays true while the dependency arrow points
// one way: src/interview/** may import from "survey-core", nothing else in src/** may import from
// src/interview/**, and no public entry point - nor the tester and the linter, the other two
// sub-bundles - must reach it, directly or transitively. Nothing here builds anything - the sources
// are read from disk and their import specifiers are inspected.

const packageRoot = findPackageRoot();
const srcDir = join(packageRoot, "src");
const interviewDir = join(srcDir, "interview");
const testerDir = join(srcDir, "tester");
const linterDir = join(srcDir, "linter");
const entriesDir = join(packageRoot, "entries");
const testsDir = join(packageRoot, "tests", "interview");
...
function findPackageRoot(): string {
  ...
  for (let i = 0; i < candidates.length; i++) {
    if (existsSync(join(candidates[i], "src", "interview", "interview.ts"))) return candidates[i];
  }
...
function isInside(dir: string, path: string): boolean {
  return toPosix(path).indexOf(toPosix(dir) + "/") === 0;
}

function isInsideInterview(path: string): boolean {
  return isInside(interviewDir, path);
}

function isInsideTests(path: string): boolean {
  return isInside(testsDir, path);
}
...
describe("The interview is isolated from the rest of the library (issue #11818)", () => {
  test("No file of src/ outside src/interview/ imports from src/interview/", () => {
    const offenders: Array<string> = [];
    getTypeScriptFiles(srcDir).forEach(file => {
      if (isInsideInterview(file)) return;
      getImports(file).forEach(record => {
        const resolved = resolveImport(record);
        if (!!resolved && isInsideInterview(resolved)) offenders.push(formatRecord(record));
      });
    });
    expect(offenders, "src/interview/ must not be imported from the rest of src/").toEqual([]);
  });

  // The tester and the linter are entry points of their own, and each of the three sub-bundles must
  // stay loadable without the other two.
  test("No public entry point reaches src/interview/, directly or transitively", () => {
    ["index.ts", "i18n.ts", "themes.ts", "tester.ts", "linter.ts"].forEach(name => {
      const entry = join(entriesDir, name);
      if (!existsSync(entry)) return;
      const chain = findInterviewImportChain(entry);
      expect(chain, "entries/" + name + " must not reach src/interview/").toEqual(undefined);
    });
  });

  test("src/interview/ imports nothing but its own files and the survey-core package", () => {
    const offenders: Array<string> = [];
    getTypeScriptFiles(interviewDir).forEach(file => {
      getImports(file).forEach(record => {
    ...
    expect(offenders, "the interview must reach the rest of the library through \"survey-core\", and must not depend on a node built-in or on any other package").toEqual([]);
  });

  // The check above already rejects every relative escape. This one names the two neighbouring
  // sub-bundles, so that the failure message says what the rule is: what they share belongs in
  // survey-core and is consumed from "survey-core", never borrowed from another sub-bundle.
  test("No file of src/interview/ imports from src/tester/ or src/linter/", () => {
    const offenders: Array<string> = [];
    getTypeScriptFiles(interviewDir).forEach(file => {
      getImports(file).forEach(record => {
        const resolved = resolveImport(record);
        if (!resolved) return;
        if (isInside(testerDir, resolved) || isInside(linterDir, resolved)) offenders.push(formatRecord(record));
      });
    });
    expect(offenders, "the sub-bundles do not depend on each other").toEqual([]);
  });

  // The unit tests read the interview the way its own sources do. A test that reached a src/ module
  // directly would bind a class the interview itself no longer imports, and the pair would only look
  // identical while the alias of vitest happens to resolve both to the same module.
  test("tests/interview/ reaches the library only through survey-core", () => {
    const offenders: Array<string> = [];
    getTypeScriptFiles(testsDir).forEach(file => {
      getImports(file).forEach(record => {
        if (record.source.charAt(0) !== ".") return;
        const resolved = resolveImport(record);
        if (!resolved || isInsideInterview(resolved) || isInsideTests(resolved)) return;
  ...
  test("Everything src/interview/ imports from survey-core is exported by it", () => {
...
function findInterviewImportChain(entry: string): Array<string> {
  ...
  while(stack.length > 0) {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      if (isInsideInterview(resolved)) return chain;
