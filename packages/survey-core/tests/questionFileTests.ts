...
describe("Survey_QuestionFile", () => {
  test("QuestionFile value initialization strings", () => {
    ...
    expect(q1.previewValue.length, "remote stored file").toLooseEqual(1);
    expect(q2.previewValue.length, "file stored as text").toLooseEqual(1);
    expect(q1.previewValue[0].content, "remote stored file content").toLooseEqual("data:image/jpeg;base64,FILECONTENT1");
    expect(q2.previewValue[0].content, "locally stored file content").toLooseEqual(survey.data.image2);
  });
  ...
  test("QuestionFile change defaultValue for allowMultiple", () => {
    ...
    expect(fileQuestion1.allowMultiple, "default is false").toLooseEqual(false);

    const prop1 = Serializer.getProperty("file", "allowMultiple");
    prop1.defaultValue = true;

    const fileQuestion2 = new QuestionFileModel("q2");
    expect(fileQuestion2.allowMultiple, "default is true").toLooseEqual(true);
  ...
  test("QuestionFile value initialization array of objects", () => {
    ...
    survey.onDownloadFile.add((survey, options) => {
      if (options.name == "image1" && options.question.name === "image1") {
        expect(q1.inputTitle).toLooseEqual(" ");
      }
      if (options.name == "image2" && options.question.name === "image2") {
        expect(q2.inputTitle).toLooseEqual(" ");
      }
      options.callback("success", "data:image/jpeg;base64,FILECONTENT1");
    });

    expect(q1.inputTitle).toLooseEqual("Choose file(s)...");
    expect(q2.inputTitle).toLooseEqual("Choose file(s)...");
    survey.data = {
    ...
    expect(q2.previewValue.length, "file stored as text").toLooseEqual(1);
    expect(q1.previewValue[0].content, "remote stored file content").toLooseEqual("data:image/jpeg;base64,FILECONTENT1");
    expect(q2.previewValue[0].content, "locally stored file content").toLooseEqual(survey.data.image2[0].content);
  });
  ...
  test("QuestionFile value initialization array of objects without onDownloadFile handler", () => {
    ...
    expect(q1.previewValue.length, "remote stored file").toLooseEqual(1);
    expect(q2.previewValue.length, "file stored as text").toLooseEqual(1);
    expect(q1.previewValue[0].content, "remote stored file content").toLooseEqual(survey.data.image1[0].content);
    expect(q2.previewValue[0].content, "locally stored file content").toLooseEqual(survey.data.image2[0].content);
  });
  ...
  test("QuestionFile upload files", () => {
    return new Promise(function(resolve) {
      ...
      survey.onValueChanged.add((survey, options) => {
        ...
        expect(q1.previewValue[0].content, "preview content 1").toLooseEqual(q1.value[0].content);
        expect(q1.previewValue[1].content, "preview content 2").toLooseEqual(q1.value[1].content);
  ...
  test("QuestionFile upload files that exceed max size - https://surveyjs.answerdesk.io/ticket/details/T994", () => {
    ...
    expect(loadedFilesCount, "no files loaded").toLooseEqual(0);

    var loadedFilesCount = 0;
    q1.loadFiles([<any>{ name: "f1", type: "t1", size: 9 }]);
    expect(q1.errors.length, "no error").toLooseEqual(0);
    expect(loadedFilesCount, "one files loaded").toLooseEqual(1);
    ...
    expect(q1.errors.length, "no error").toLooseEqual(0);
    expect(loadedFilesCount, "two files loaded").toLooseEqual(1);
  ...
  test("QuestionFile upload files that exceed maxFiles, Issue #10549", () => {
    ...
    const survey = new SurveyModel(json);
    const q1: QuestionFileModel = <any>survey.getQuestionByName("image1");
    expect(q1.maxFiles, "maxFiles is set correctly").toLooseEqual(2);
    ...
    <any>{ name: "f2", type: "t2" }]);
    expect(q1.errors.length, "one error").toLooseEqual(0);
    expect(loadedFilesCount, "two files loaded").toLooseEqual(2);
    ...
    expect(q1.errors.length, "one error").toLooseEqual(1);
    expect(q1.errors[0].locText.text, "error text").toLooseEqual("The maximum number of files you can upload is 2.");
    expect(loadedFilesCount, "no files loaded").toLooseEqual(0);
    ...
    q1.loadFiles([<any>{ name: "f1", type: "t1" }]);
    expect(q1.errors.length, "no error").toLooseEqual(0);
    expect(loadedFilesCount, "one file loaded").toLooseEqual(1);
  ...
  test("QuestionFile process errors during files uploading - https://surveyjs.answerdesk.io/ticket/details/T1075", async () => {
    return new Promise(function(resolve) {
      ...
      expect(state).toLooseEqual("");
      expect(stateSec).toLooseEqual("");
  ...
  test("QuestionFile replace file for single file mode", () => {
    ...
    q1.loadFiles(files1);
    expect(q1.value.length, "first file").toLooseEqual(1);
    expect(q1.value[0].name, "first file name").toLooseEqual("f1");

    var files2: any = [{ name: "f2", type: "t2", size: 100000 }];
    q1.loadFiles(files2);
    expect(q1.value.length, "the only single file").toLooseEqual(1);
    expect(q1.value[0].name, "second file name").toLooseEqual("f2");
  });
  test("QuestionFile in panel dynamic in preview mode", () => {
    ...
    expect(panel.panelCount, "One panel").toLooseEqual(1);
    var fileQuestion = <QuestionFileModel>panel.panels[0].questions[0];
    expect(fileQuestion.value, "Question file name").toLooseEqual("someId");
  });
   ...
   */
    expect(q.confirmRemoveMessage, "The remove file confirmation message template default").toLooseEqual(getLocaleString("confirmRemoveFile"));
    q.confirmRemoveMessage += "_new";
    expect(q.confirmRemoveMessage, "The remove file confirmation message template new").toLooseEqual(getLocaleString("confirmRemoveFile") + "_new");
    /**
    * The remove all files confirmation message.
    */
    expect(q.confirmRemoveAllMessage, "The remove all files confirmation message default").toLooseEqual(getLocaleString("confirmRemoveAllFiles"));
    q.confirmRemoveAllMessage += "_new";
    expect(q.confirmRemoveAllMessage, "The remove all files confirmation message new").toLooseEqual(getLocaleString("confirmRemoveAllFiles") + "_new");
    /**
    * The no file chosen caption for modern theme.
    */
    expect(q.noFileChosenCaption, "The no file chosen caption for modern theme default").toLooseEqual(getLocaleString("noFileChosen"));
    q.noFileChosenCaption += "_new";
    expect(q.noFileChosenCaption, "The no file chosen caption for modern theme new").toLooseEqual(getLocaleString("noFileChosen") + "_new");
    /**
    * The choose files button caption for modern theme.
    */
    expect(q.chooseButtonCaption, "The choose files button caption for modern theme default").toLooseEqual(getLocaleString("chooseFileCaption"));
    q.chooseButtonCaption += "_new";
    expect(q.chooseButtonCaption, "The choose files button caption for modern theme new").toLooseEqual(getLocaleString("chooseFileCaption") + "_new");
    /**
    * The clean files button caption.
    */
    expect(q.clearButtonCaption, "The clean files button caption default").toLooseEqual(getLocaleString("clearCaption"));
    q.clearButtonCaption += "_new";
    expect(q.clearButtonCaption, "The clean files button caption new").toLooseEqual(getLocaleString("clearCaption") + "_new");
    /**
    * The remove file button caption.
    */
    expect(q.removeFileCaption, "The remove file button caption default").toLooseEqual(getLocaleString("removeFileCaption"));
    q.removeFileCaption += "_new";
    expect(q.removeFileCaption, "The remove file button caption new").toLooseEqual(getLocaleString("removeFileCaption") + "_new");
    /**
    * The loading file input title.
    */
    expect(q.loadingFileTitle, "The loading file input title default").toLooseEqual(getLocaleString("loadingFile"));
    q.loadingFileTitle += "_new";
    expect(q.loadingFileTitle, "The loading file input title new").toLooseEqual(getLocaleString("loadingFile") + "_new");
    /**
   * The choose file input title.
   */
    expect(q.chooseFileTitle, "The choose file input title default").toLooseEqual(getLocaleString("chooseFile"));
    q.chooseFileTitle += "_new";
    expect(q.chooseFileTitle, "The choose file input title new").toLooseEqual(getLocaleString("chooseFile") + "_new");
  ...
  test("Check choose button text", () => {
    ...
    expect(q.chooseButtonText).toLooseEqual("Select File");
    q.value = [{
      content: "https://api.surveyjs.io/public/v1/Survey/file?filePath=dcc81e2a-586f-45dd-b734-ee86bcbad8db.png",
      name: "name.png",
      type: "image/png"
    }];
    expect(q.chooseButtonText).toLooseEqual("Replace file");
  ...
  test("check file d&d", () => {
    ...
    q.onDragEnter(event);
    expect(q["dragCounter"]).toLooseEqual(1);
    expect(q.isDragging).toLooseEqual(true);

    q.onDragOver(event);
    expect(event.dataTransfer.dropEffect).toLooseEqual("copy");
    expect(q.isDragging).toLooseEqual(true);

    q.onDragLeave(event);
    expect(q["dragCounter"]).toLooseEqual(0);
    expect(q.isDragging).toLooseEqual(false);
    ...
    q.onDragEnter(event);
    expect(q["dragCounter"]).toLooseEqual(2);
    expect(q.isDragging).toLooseEqual(true);
    ...
    expect(q["dragCounter"]).toLooseEqual(0);
    expect(q.isDragging).toLooseEqual(false);
    expect(onChangeCalledCount).toLooseEqual(1);
  });
  ...
  test("check file d&d readonly", () => {
    ...
    var q: QuestionFileModel = <QuestionFileModel>survey.getQuestionByName("file1");
    let onChangeCalledCount = 0;
    expect(q["canDragDrop"](), "canDragDrop").toLooseEqual(true);
    q["onChange"] = () => { onChangeCalledCount++; };
    const event = { preventDefault: () => { }, dataTransfer: { dropEffect: "none", files: [{ type: "ext", name: "test", content: "test_content" }] } };
    const checkDD = () => {
      q.onDragOver(event);
      expect(event.dataTransfer.dropEffect).toLooseEqual("none");
      expect(q.isDragging).toLooseEqual(false);
      ...
      q.onDrop(event);
      expect(q.isDragging).toLooseEqual(false);
      expect(onChangeCalledCount).toLooseEqual(0);
    };
    q.readOnly = true;
    checkDD();
  ...
  test("file.clearButtonCaption localization", () => {
    ...
    var q: QuestionFileModel = <QuestionFileModel>survey.getQuestionByName("file1");
    expect(q.clearButtonCaption).toLooseEqual("Clear");
    survey.locale = "fr";
    expect(q.clearButtonCaption).toLooseEqual("Vider");
    survey.locale = "";
  });
  ...
  test("Question File responsive", () => {
    ...
    expect(q1.fileRootCss).toLooseEqual("sd-file");
    q1.isMobile = true;
    q1.pageSize = 1;
    expect(q1.fileRootCss).toLooseEqual("sd-file m");
    ...
    expect(q1.fileNavigatorVisible).toLooseEqual(true);
    const fileIndexAction = q1.fileNavigator.getActionById("fileIndex");
    expect(fileIndexAction.title).toLooseEqual("2 of 2");
    const nextFileAction = q1.fileNavigator.getActionById("nextPage");
    nextFileAction.action();
    expect(fileIndexAction.title).toLooseEqual("1 of 2");
    nextFileAction.action();
    expect(fileIndexAction.title).toLooseEqual("2 of 2");
    ...
    expect(q1.pages.length).toLooseEqual(2);
    expect(q1.pages[0].items.length).toLooseEqual(1);
    expect(q1.pages[1].items.length).toLooseEqual(1);

    expect(q1.pages[0].css).toLooseEqual("sd-file__page");
    expect(q1.pages[1].css).toLooseEqual("sd-file__page");
  ...
  test("preview item index on last file removed", () => {
    ...
    expect(q.indexToShow, "Start from 0").toLooseEqual(0);
    const fileIndexAction = q.fileNavigator.getActionById("fileIndex");
    expect(fileIndexAction.title, "Initial title").toLooseEqual("1 of 3");
    const prevFileAction = q.fileNavigator.getActionById("prevPage");
    prevFileAction.action();
    expect(q.indexToShow, "We're on 3rd image").toLooseEqual(2);
    expect(fileIndexAction.title, "We're on the last item").toLooseEqual("3 of 3");
    q.doRemoveFile(q.value[2], { stopPropagation: () => {} });
    expect(q.indexToShow, "We're on 2nd image").toLooseEqual(1);
    expect(fileIndexAction.title, "We're on the last item again").toLooseEqual("2 of 2");
  });
  ...
  test("QuestionFile upload state sequence", () => {
    ...
    expect(q1.isEmpty()).toBeFalsy();
    expect(q1.value.length).toLooseEqual(1);
    expect(q1.value[0].content).toLooseEqual("f1_url");
    expect(state).toLooseEqual("->loading->loaded");

    q1.clear();
    expect(state).toLooseEqual("->loading->loaded->empty");
  });
  ...
  test("Check download file event", () => {
    ...
    expect(log, "Every file should be loaded only once").toLooseEqual("->file1.png->file2.png->file3.png");
    expect(q.value.length, "Question value contains 3 files").toLooseEqual(3);
    expect(q.currentState, "The loaded state after data assigned").toLooseEqual("loaded");
  });
  ...
  test("QuestionFile download file content on preview, #1", () => {
    ...
    expect(downloadLog).toLooseEqual("->f1");
  ...
  test("File Question on Smaller Screens: navigation bar doesn't appear when the survey.onDownloadFile event is used", () => {
    return new Promise(function(resolve) {
      ...
      expect(fileIndexAction.title).toLooseEqual("1 of 0");
      expect(question.containsMultiplyFiles).toLooseEqual(false);
      expect(question.fileNavigatorVisible).toLooseEqual(false);
  ...
  test("Check file question navigator with different items count visible", () => {
    ...
    expect(question.fileNavigatorVisible).toLooseEqual(true);
    expect(question.indexToShow).toLooseEqual(1);
    expect(fileIndexAction.title).toLooseEqual("2 of 2");
    expect(question.pages.length).toLooseEqual(2);
    expect(question.pages[0].items.length).toLooseEqual(3);
    expect(question.pages[1].items.length).toLooseEqual(1);
    prevFileAction.action();
    expect(question.indexToShow).toLooseEqual(0);
    expect(fileIndexAction.title).toLooseEqual("1 of 2");
    expect(question.pages[0].css).toLooseEqual("sd-file__page");
    expect(question.pages[1].css).toLooseEqual("sd-file__page");
    nextFileAction.action();
    expect(question.indexToShow).toLooseEqual(1);
    expect(fileIndexAction.title).toLooseEqual("2 of 2");
    expect(question.pages[0].css).toLooseEqual("sd-file__page");
    ...
    expect(question.indexToShow).toLooseEqual(2);
    expect(fileIndexAction.title).toLooseEqual("3 of 3");
    ...
    expect(fileIndexAction.title).toLooseEqual("2 of 2");
    expect(question.pages.length).toLooseEqual(2);
    expect(question.pages[0].items.length).toLooseEqual(2);
    expect(question.pages[1].items.length).toLooseEqual(2);
    question.pageSize = 1;
    expect(question.indexToShow).toLooseEqual(1);
    expect(fileIndexAction.title).toLooseEqual("2 of 4");
    expect(question.pages.length).toLooseEqual(4);
    expect(question.pages[0].items.length).toLooseEqual(1);
    expect(question.pages[1].items.length).toLooseEqual(1);
    expect(question.pages[2].items.length).toLooseEqual(1);
    expect(question.pages[3].items.length).toLooseEqual(1);
  });
  test("Check file question processResponsiveness method", () => {
    ...
    expect(question.pageSize).toLooseEqual(3);

    question["calculatedGapBetweenItems"] = 32;
    question["calculatedItemWidth"] = 96;
    question["processResponsiveness"](0, 250);
    expect(question.pageSize).toLooseEqual(2);
    ...
    question["calculatedItemWidth"] = 50;
    question["processResponsiveness"](0, 250);
    expect(question.pageSize).toLooseEqual(4);

    question["calculatedGapBetweenItems"] = 8;
    question["calculatedItemWidth"] = 50;
    question["processResponsiveness"](0, 58);
    expect(question.pageSize).toLooseEqual(1);
  ...
  test("QuestionFile download file content on preview, #2", () => {
    ...
    expect(q1.locRenderedPlaceholder.renderedHtml.substring(0, 4), "q1 => drag").toLooseEqual("Drag");
    expect(q2.locRenderedPlaceholder.renderedHtml.substring(0, 2), "q2 => no file").toLooseEqual("No");
    q1.readOnly = true;
    q2.readOnly = false;
    expect(q1.locRenderedPlaceholder.renderedHtml.substring(0, 2), "q1, readOnly => no file").toLooseEqual("No");
    expect(q2.locRenderedPlaceholder.renderedHtml.substring(0, 4), "q2, not readOnly=> drag").toLooseEqual("Drag");
  });
  ...
  test("QuestionFile current mode property, camera is not available", () => {
    ...
    expect(survey.getQuestionByName("q1").currentMode, "#1.1").toLooseEqual("file");
    expect(survey.getQuestionByName("q2").currentMode, "#1.2").toLooseEqual("file");
    expect(survey.getQuestionByName("q3").currentMode, "#1.3").toLooseEqual("camera");
    expect(survey.getQuestionByName("q4").currentMode, "#1.4").toLooseEqual("file-camera");
    expect(callbacks.length > 0, "callbacks are set").toBeTruthy();
    callbacks.forEach(cb => cb([]));
    expect(survey.getQuestionByName("q1").currentMode, "#1").toLooseEqual("file");
    expect(survey.getQuestionByName("q2").currentMode, "#2").toLooseEqual("file");
    expect(survey.getQuestionByName("q3").currentMode, "#3").toLooseEqual("file");
    expect(survey.getQuestionByName("q4").currentMode, "#4").toLooseEqual("file");
    callbacks.splice(0, callbacks.length);
    survey.getQuestionByName("q1").sourceType = "camera";
    expect(survey.getQuestionByName("q1").currentMode, "#5").toLooseEqual("file");
    callbacks.forEach(cb => cb([]));
    expect(survey.getQuestionByName("q1").currentMode, "#6").toLooseEqual("file");
  ...
  test("QuestionFile current mode property, camera is available", () => {
    ...
    expect(survey.getQuestionByName("q1").currentMode, "#1").toLooseEqual("file");
    expect(survey.getQuestionByName("q2").currentMode, "#2").toLooseEqual("file");
    expect(survey.getQuestionByName("q3").currentMode, "#3").toLooseEqual("camera");
    expect(survey.getQuestionByName("q4").currentMode, "#4").toLooseEqual("file-camera");
    callbacks.splice(0, callbacks.length);
    survey.getQuestionByName("q1").sourceType = "camera";
    expect(survey.getQuestionByName("q1").currentMode, "#5").toLooseEqual("camera");
    callbacks.forEach(cb => cb(devices));
    expect(survey.getQuestionByName("q1").currentMode, "#6").toLooseEqual("camera");
    Camera.mediaDevicesCallback = undefined;
  });
  test("QuestionFile check file actions visibility when camera is available from start point", () => {
    ...
    expect(survey.getQuestionByName("q1").currentMode).toLooseEqual("camera");
    ...
    expect(survey.getQuestionByName("q2").currentMode).toLooseEqual("file");
    ...
    expect(survey.getQuestionByName("q3").currentMode).toLooseEqual("file-camera");
  ...
  test("new Camera().getMediaConstraints", () => {
    ...
    expect(mConst.video.deviceId.exact, "Device is correct").toLooseEqual(2);
    Camera.setCameraList(createDevices([{ label: "abd" }, { label: "environment" }, { label: "user" }]));
    mConst = new Camera().getMediaConstraints();
    expect(mConst.video.deviceId.exact, "Device is correct").toLooseEqual(3);
  ...
  test("new Camera().flip", () => {
    expect(new Camera().canFlip(), "There is no devices").toLooseEqual(false);
    Camera.setCameraList(createDevices([{ label: "abd" }]));
    expect(new Camera().canFlip(), "There is one device").toLooseEqual(false);
    Camera.setCameraList(createDevices([{ label: "abd" }, { label: "environment" }, { label: "user" }]));
    expect(new Camera().canFlip(), "There are 3 devices").toLooseEqual(true);
    let mConst: any = new Camera().getMediaConstraints();
    expect(mConst.video.deviceId.exact, "Device is correct").toLooseEqual(3);
    new Camera().flip();
    mConst = new Camera().getMediaConstraints();
    expect(mConst.video.deviceId.exact, "Flip #1").toLooseEqual(2);
    new Camera().flip();
    mConst = new Camera().getMediaConstraints();
    expect(mConst.video.deviceId.exact, "Flip #2").toLooseEqual(1);
    new Camera().flip();
    mConst = new Camera().getMediaConstraints();
    expect(mConst.video.deviceId.exact, "Flip #2").toLooseEqual(3);
    Camera.clear();
  });
  test("Check file question change camera action", () => {
    ...
    q1["camera"].flip();
    expect(Camera["cameraIndex"]).toLooseEqual(1);
    q1["camera"].flip();
    expect(Camera["cameraIndex"]).toLooseEqual(2);
    q1["camera"].flip();
    expect(Camera["cameraIndex"]).toLooseEqual(0);
    ...
    q1["camera"]["updateCanFlipValue"]();
    expect(changeCameraAction.visible).toBeTruthy();
    expect(Camera["cameraFacingMode"]).toLooseEqual("user");
    q1["camera"].flip();
    expect(Camera["cameraFacingMode"]).toLooseEqual("environment");
    q1["camera"].flip();
    expect(Camera["cameraFacingMode"]).toLooseEqual("user");
    Camera.clear();
  });
  ...
  test("QuestionFile stop playing video on hiding question", () => {
    ...
    let q1 = <QuestionFileModel>survey.getQuestionByName("q1");
    q1.setPropertyValue("isPlayingVideo", true);
    expect(q1.isPlayingVideo).toLooseEqual(true);
    q1.visible = false;
    expect(q1.isPlayingVideo, "question invisible").toLooseEqual(false);
    q1.visible = true;
    q1.setPropertyValue("isPlayingVideo", true);
    q1.collapse();
    expect(q1.isPlayingVideo, "question content is collapsed").toLooseEqual(false);
    ...
    expect(q1.isPlayingVideo).toLooseEqual(true);
    panel.visible = false;
    expect(q1.isPlayingVideo, "panel invisible").toLooseEqual(false);
    q1.setPropertyValue("isPlayingVideo", true);
    panel.collapse();
    expect(q1.isPlayingVideo, "panel content is collapsed").toLooseEqual(false);
  });
  test("QuestionFile stop playing video on going to another page or complete", () => {
    ...
    expect(q1.isPlayingVideo).toLooseEqual(true);
    survey.nextPage();
    expect(q1.isPlayingVideo, "Go to next page").toLooseEqual(false);

    survey.currentPageNo = 1;
    q1.setPropertyValue("isPlayingVideo", true);
    expect(q1.isPlayingVideo).toLooseEqual(true);
    survey.currentPageNo = 0;
    expect(q1.isPlayingVideo, "Go to prev page").toLooseEqual(false);
    ...
    expect(q1.isPlayingVideo).toLooseEqual(true);
    survey.doComplete();
    expect(q1.isPlayingVideo, "complete survey").toLooseEqual(false);
  });
  test("QuestionFile stop playing video when question's html element is destroyed", () => {
    ...
    survey.nextPage();
    expect(q1.isPlayingVideo).toLooseEqual(false);
  });
  ...
  test("QuestionFile check actions container", () => {
    ...
    expect(startCameraAction.title).toLooseEqual("take_picture_test");
    expect(cleanAction.title).toLooseEqual("clear_test");
  ...
  test("QuestionFile check renderedPlaceholder in different modes", () => {
    ...
    expect(q1.locRenderedPlaceholder.renderedHtml).toLooseEqual("file_mod_placeholder");
    q1.setPropertyValue("currentMode", "camera");
    expect(q1.locRenderedPlaceholder.renderedHtml).toLooseEqual("camera_mod_placeholder");
    q1.setPropertyValue("currentMode", "file-camera");
    expect(q1.locRenderedPlaceholder.renderedHtml).toLooseEqual("both_mod_placeholder");
  });
  ...
  test("QuestionFile allowImagesPreview and allowCameraAccess", () => {
    ...
    expect(prop2.visible).toLooseEqual(false);
  });
  ...
  test("Acton takePhoto should be localized", () => {
    ...
    const question = <QuestionFileModel>survey.getAllQuestions()[0];
    expect(question.takePictureAction.title, "en").toLooseEqual("Take Photo");
    ...
    expect(startCameraAction.title, "en").toLooseEqual("Take Photo");
    expect(cleanAction.title, "en").toLooseEqual("Clear");
    survey.locale = "de";
    expect(question.takePictureAction.title, "de").toLooseEqual("Foto machen");
    expect(startCameraAction.title, "de").toLooseEqual("Foto machen");
    expect(cleanAction.title, "de").toLooseEqual("Auswahl entfernen");
  });
  ...
  test("Choose file action should have disabled class", () => {
    ...
    survey.css = defaultCss;
    const question = <QuestionFileModel>survey.getAllQuestions()[0];
    expect(question.getChooseFileCss(), "Disabled").toLooseEqual("sd-file__choose-btn sd-file__choose-file-btn--disabled sd-action sd-file__choose-btn--text sd-action--disabled");
    survey.readOnly = false;
    expect(question.getChooseFileCss(), "Enabled").toLooseEqual("sd-file__choose-btn sd-action sd-file__choose-btn--text");
  ...
  test("Bug #8242: currentMode is set incorrectly when file question is located inside matrixdynamic", () => {
    ...
    expect(getFileQuestionFromRow(0).currentMode).toLooseEqual("file-camera");
    expect(getFileQuestionFromRow(0).actionsContainer.getActionById("sv-file-start-camera").visible).toBeTruthy();
    question.addRowUI();
    expect(getFileQuestionFromRow(1).currentMode).toLooseEqual("file-camera");
    expect(getFileQuestionFromRow(1).actionsContainer.getActionById("sv-file-start-camera").visible).toBeTruthy();
    question.addRowUI();
    expect(getFileQuestionFromRow(2).currentMode).toLooseEqual("file-camera");
  ...
  test("Check renderedPages property", () => {
    ...
    expect(q1.pages.length).toLooseEqual(1);
    expect(q1.pages[0].items.length).toLooseEqual(1);
    expect(q1.pages[0].items[0].name).toLooseEqual("item1");

    survey.data = {
      image1: [{ name: "item1" }, { name: "item2" }, { name: "item3" }],
    };
    q1.indexToShow = 0;
    expect(q1.pages.length).toLooseEqual(3);
    expect(q1.pages[0].items.length).toLooseEqual(1);
    expect(q1.pages[0].items[0].name).toLooseEqual("item1");

    expect(q1.pages[1].items.length).toLooseEqual(1);
    expect(q1.pages[1].items[0].name).toLooseEqual("item2");

    expect(q1.pages[2].items.length).toLooseEqual(1);
    expect(q1.pages[2].items[0].name).toLooseEqual("item3");

    expect(q1.renderedPages.length).toLooseEqual(1);
    expect(q1.renderedPages[0].items.length).toLooseEqual(1);
    expect(q1.renderedPages[0].items[0].name).toLooseEqual("item1");
    const prevFileAction = q1.fileNavigator.getActionById("prevPage");
    prevFileAction.action();
    expect(q1.navigationDirection).toLooseEqual("left");
    expect(q1.renderedPages.length).toLooseEqual(1);
    expect(q1.renderedPages[0].items.length).toLooseEqual(1);
    expect(q1.renderedPages[0].items[0].name).toLooseEqual("item3");

    const nextFileAction = q1.fileNavigator.getActionById("nextPage");
    nextFileAction.action();
    expect(q1.navigationDirection).toLooseEqual("right");
    expect(q1.renderedPages.length).toLooseEqual(1);
    expect(q1.renderedPages[0].items.length).toLooseEqual(1);
    expect(q1.renderedPages[0].items[0].name).toLooseEqual("item1");
    ...
    expect(q1.renderedPages.length).toLooseEqual(1);
    expect(q1.renderedPages[0].items.length).toLooseEqual(1);
    expect(q1.renderedPages[0].items[0].name).toLooseEqual("item2");
    ...
    q1.removeFile("item1");
    expect(q1.navigationDirection).toLooseEqual(undefined);
    expect(q1.pages.length).toLooseEqual(2);
    expect(q1.pages[0].items.length).toLooseEqual(1);
    expect(q1.pages[0].items[0].name).toLooseEqual("item2");

    expect(q1.pages[1].items.length).toLooseEqual(1);
    expect(q1.pages[1].items[0].name).toLooseEqual("item3");
    ...
    nextFileAction.action();
    q1.removeFile("item3");
    expect(q1.navigationDirection).toLooseEqual("left-delete");
    expect(q1.pages.length).toLooseEqual(1);
    expect(q1.pages[0].items.length).toLooseEqual(1);
    expect(q1.pages[0].items[0].name).toLooseEqual("item2");
  ...
  test("Check pageAnimationOptions", () => {
    ...
    //check enter options
    expect(fpNavigationOptions.getEnterOptions(q1.pages[0]).cssClass).toLooseEqual("");
    q1.navigationDirection = "left";
    expect(fpNavigationOptions.getEnterOptions(q1.pages[0]).cssClass).toLooseEqual("sd-file__page--enter-from-left");
    q1.navigationDirection = "right";
    expect(fpNavigationOptions.getEnterOptions(q1.pages[0]).cssClass).toLooseEqual("sd-file__page--enter-from-right");
    ...
    expect(fpNavigationOptions.getLeaveOptions(q1.pages[0]).cssClass).toLooseEqual("");
    q1.navigationDirection = "left-delete";
    expect(fpNavigationOptions.getLeaveOptions(q1.pages[0]).cssClass).toLooseEqual("");
    q1.navigationDirection = "left";
    expect(fpNavigationOptions.getLeaveOptions(q1.pages[0]).cssClass).toLooseEqual("sd-file__page--leave-to-right");
    q1.navigationDirection = "right";
    expect(fpNavigationOptions.getLeaveOptions(q1.pages[0]).cssClass).toLooseEqual("sd-file__page--leave-to-left");
  ...
  test("check source type option in onUploadFiles event", () => {
    ...
    expect(log).toLooseEqual("->file: from_chooser");
    log = "";
    file.setPropertyValue("isPlayingVideo", true);
    file.snapPicture();
    expect(log).toLooseEqual("->camera: snap_picture.png");
    log = "";
    ...
    expect(log).toLooseEqual("->signature: q2.png");
  });
  ...
  test("questionFile acceptedCategories property, Issue#10602", () => {
    ...
    expect(q1.renderedAcceptedTypes, "#1").toLooseEqual(documentTypes);
    expect(q2.renderedAcceptedTypes, "#2").toLooseEqual(documentTypes + ",.jpeg");
    expect(q3.renderedAcceptedTypes, "#3").toLooseEqual(".pdf,.jpeg");
    expect(q4.renderedAcceptedTypes, "#4").toBe(undefined);

    expect(q1.acceptedCategories.length, "q1 categories count").toLooseEqual(1);
    expect(q2.acceptedCategories.length, "q2 categories count").toLooseEqual(2);
    expect(q2.acceptedCategories[1], "q2 second category").toLooseEqual("custom");
    expect(q3.acceptedCategories.length, "q3 categories count").toLooseEqual(1);
    expect(q3.acceptedCategories[0], "q3 first category").toLooseEqual("custom");
    expect(q4.acceptedCategories.length, "q4 categories count").toLooseEqual(0);
    ...
    expect(q4.acceptedTypes, "q4 acceptedTypes after setting archive category").toLooseEqual(undefined);
  });
  test("questionFile acceptedTypes property as an array, Bug#10627", () => {
    ...
    const q1 = survey.getQuestionByName("q1") as QuestionFileModel;
    const q2 = survey.getQuestionByName("q2") as QuestionFileModel;
    expect(q1.renderedAcceptedTypes, "#1").toLooseEqual(documentTypes + ",.jpeg");
    expect(q2.renderedAcceptedTypes, "#2").toLooseEqual(".pdf,.jpeg");
    ...
    expect(q2.renderedAcceptedTypes, "#4").toLooseEqual(".doc,.txt");
  });
  test("questionFile acceptedTypes property, could not remove custom, Bug#10647", () => {
    ...
    expect(q1.renderedAcceptedTypes, "q1 renderedAcceptedTypes").toLooseEqual(documentTypes);
    expect(q2.renderedAcceptedTypes, "q2 renderedAcceptedTypes").toLooseEqual(undefined);
  });
});
