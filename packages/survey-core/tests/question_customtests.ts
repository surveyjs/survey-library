...
describe("custom questions", () => {
  ...
  // VITEST-MIGRATION: MANUAL -- OTHER: deepEqual on observable arrays surfaces extra enumerable properties (`push`, etc.). Vitest's toEqual is strict; QUnit's deepEqual ignored them.
  test.skip("Single: with checkbox & showOtherItem, Bug#9929", () => {
    ComponentCollection.Instance.add({
      name: "test",
  ...
  // VITEST-MIGRATION: MANUAL -- DOM_API: relies on document.activeElement focus tracking that jsdom does not implement correctly.
  test.skip("Single: Do not focus element on setting defaultValue & on setting value to survey.data, Bug#10016", () => {
  ...
  // VITEST-MIGRATION: MANUAL -- DOM_API: relies on document.activeElement focus tracking that jsdom does not implement correctly.
  test.skip("Composite: Do not focus element on setting defaultValue & on setting value to survey.data, Bug#10016", () => {
    const oldFunc = SurveyElement.FocusElement;
    let counter = 0;
    SurveyElement.FocusElement = function (elId: string): boolean {
      counter ++;
