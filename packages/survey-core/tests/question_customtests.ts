...
QUnit.test("Single: supportAutoAdvance/supportGoNextPageAutomatic, bug#10149", function (assert) {
  ...
  const q1 = <QuestionCustomModel>survey.getQuestionByName("q1");
  assert.equal(q1.supportGoNextPageAutomatic(), false, "supportAutoAdvance #1");
  q1.contentQuestion.onMouseDown();
  assert.equal(q1.supportGoNextPageAutomatic(), true, "supportAutoAdvance #2");
  q1.contentQuestion.value = "other";
  assert.equal(q1.supportGoNextPageAutomatic(), false, "supportAutoAdvance #3");
  ComponentCollection.Instance.clear();
});
