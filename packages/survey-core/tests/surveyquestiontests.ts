...
QUnit.test("Matrix Question supportAutoAdvanced property", function (
  assert
) {
  var matrix = new QuestionMatrixModel("q1");
  matrix.rows = ["row1", "row2"];
  matrix.columns = ["col1", "col2"];
  assert.equal(matrix.supportAutoAdvanced(), false, "Rows are not set");
  matrix.onMouseDown();
  matrix.value = { row1: "col1" };
  assert.equal(
    matrix.supportAutoAdvanced(),
    false,
  ...
  matrix.onMouseDown();
  assert.equal(matrix.supportAutoAdvanced(), true, "Both rows are set");
});
...
QUnit.test("Multiple Text Question: support goNextPageAutomatic", function (
  ...
  assert.equal(mText.supportAutoAdvanced(), false, "all text are empty");
  mText.value = { tex1: "val1" };
  assert.equal(
    mText.supportAutoAdvanced(),
    false,
...
QUnit.test("Use timer to go next page", function (assert) {
  ...
  assert.equal(question.supportAutoAdvanced(), true, "questio support go next page automatic");
  question.value = 1;
...
QUnit.test("QuestionImagePickerModel.supportAutoAdvanced", function (assert) {
  const q = new QuestionImagePickerModel("q");
  assert.equal(q.supportAutoAdvanced(), true, "It supports by default");
  q.multiSelect = true;
  assert.equal(q.supportAutoAdvanced(), false, "It doesn't support it for multiselect");
  q.multiSelect = false;
  assert.equal(q.supportAutoAdvanced(), true, "multiselect is false");
});
QUnit.test("QuestionTextModel.supportAutoAdvanced", function (assert) {
  const q = new QuestionTextModel("q");
  assert.equal(q.supportAutoAdvanced(), true, "It supports by default");
  q.inputType = "date";
  assert.equal(q.supportAutoAdvanced(), false, "Do not support for date");
  q.inputType = "text";
  assert.equal(q.supportAutoAdvanced(), true, "Default inputType again");
  q.textUpdateMode = "onTyping";
  assert.equal(q.supportAutoAdvanced(), false, "textUpdateMode = 'onTyping'");
  q.textUpdateMode = "onBlur";
  assert.equal(q.supportAutoAdvanced(), true, "textUpdateMode = 'onBlur'");
});
