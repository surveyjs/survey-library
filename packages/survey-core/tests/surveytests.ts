...
QUnit.test("question.no and survey.questionStartIndex = 'A.1' and showQuestionNumbers 'recursive'", function (assert) {
  const survey = new SurveyModel({
    pages: [{
      elements: [
        {
  ...
  assert.equal(q1.no, "A.1", "q1, #1");
  assert.equal(q2.no, "A.2", "q2, #1");
  assert.equal(p1.no, "A.3", "panel1, #1");
  assert.equal(q3.no, "A.3.1", "q3, #1");
  assert.equal(q4.no, "A.3.2", "q4, #1");
...
QUnit.test("valueName for matrix dynamic and panel dynamic with different question and display text processing, bug# https://surveyjs.answerdesk.io/ticket/details/T2053", (assert) => {
  const survey = new SurveyModel({
    elements: [
      {
        name: "matrix1",
  ...
  assert.deepEqual(survey.data, { shared: [{ elementId: 1, col1: 1 }] }, "set matrix1 col1");
  panel1.panels[0].getQuestionByName("ed1").value = 1;
  assert.deepEqual(matrix1.getDisplayValue(false), [{ elementId: "1", col1: "Item 1", ed1: "Item 10" }]);
  assert.deepEqual(panel1.getDisplayValue(false), [{ elementId: "1", col1: "Item 1", ed1: "Item 10" }]);
  assert.equal(panel1.panels[0].locTitle.renderedHtml, "Item 1", "Get the display text");
});
...
QUnit.test("Check default navigation items relevance", function (assert) {
  ...
  survey.locale = "ru";
  assert.equal(action.title, "Начать"); // eslint-disable-line surveyjs/eslint-plugin-i18n/only-english-or-code
  survey.locale = "en";
...
QUnit.test("Check survey getRootCss function - defaultCss", function (assert) {
  ...
  survey.css = defaultCss;
  assert.equal(survey.getRootCss(), "sd-root-modern sd-theme-root sd-progress--pages sd-root-modern--full-container");

  survey.fitToContainer = false;
  assert.equal(survey.getRootCss(), "sd-root-modern sd-theme-root sd-progress--pages");

  survey.setIsMobile(true);
  survey.fitToContainer = true;
  assert.equal(survey.getRootCss(), "sd-root-modern sd-theme-root sd-progress--pages sd-root-modern--mobile sd-root-modern--full-container");

  survey.fitToContainer = false;
  assert.equal(survey.getRootCss(), "sd-root-modern sd-theme-root sd-progress--pages sd-root-modern--mobile");

  survey.readOnly = true;
  survey.fitToContainer = true;
  assert.equal(survey.getRootCss(), "sd-root-modern sd-theme-root sd-progress--pages sd-root-modern--mobile sd-root--readonly sd-root-modern--full-container");

  survey.fitToContainer = false;
  assert.equal(survey.getRootCss(), "sd-root-modern sd-theme-root sd-progress--pages sd-root-modern--mobile sd-root--readonly");

  survey.readOnly = false;
  survey.setIsMobile(false);
  survey["isCompact"] = true;
  assert.equal(survey.getRootCss(), "sd-root-modern sd-theme-root sd-progress--pages sd-root--compact");

  survey.fitToContainer = true;
  assert.equal(survey.getRootCss(), "sd-root-modern sd-theme-root sd-progress--pages sd-root--compact sd-root-modern--full-container");
...
QUnit.test("check titleNumInline cssClass", function (assert) {
  ...
  const question = survey.getQuestionByName("q1");
  const panel = survey.getPanelByName("p1");
  assert.ok(question.cssTitle.includes(customInlineClass), "#1");
  assert.ok(panel.cssTitle.includes(customInlineClass), "#2");
  survey.questionStartIndex = "1.1";
  assert.notOk(question.cssTitle.includes(customInlineClass), "#3");
  assert.notOk(panel.cssTitle.includes(customInlineClass), "#4");
});
QUnit.test("check titleNumInline cssClass, Bug#10775", function (assert) {
  const survey = new SurveyModel({
    showQuestionNumbers: "on",
  ...
  const q1 = survey.getQuestionByName("q1");
  const q2 = survey.getQuestionByName("q2");
  assert.ok(q1.cssTitle.includes(customInlineClass), "q1 titleNumInline class check #1");
  q1.value = 1;
  assert.ok(q2.isVisible);
  assert.ok(q2.cssTitle.includes(customInlineClass), "q2 titleNumInline class check #2");
  q1.value = 2;
  assert.notOk(q2.isVisible);
  q1.value = 1;
  assert.ok(q2.isVisible);
  assert.ok(q2.cssTitle.includes(customInlineClass), "q2 titleNumInline class check #3");
});
...
QUnit.test("Search disabled after change popup displayMode", function (assert) {
  ...
  popup.show();
  assert.equal(list.showFilter, true, "#5");
  popup.hide();
...
QUnit.test("questionPerPage & asyc validation & using panel, Bug#10163 #2 (panel)", (assert) => {
  let returnResultFunc = undefined;
  function isItCorrect([value]) {
    returnResultFunc = this.returnResult;
  }
  ...
  const q1 = survey.getQuestionByName("q1");
  assert.equal(survey.currentSingleElement.name, "panel1", "current question, #1");
  q1.value = "abcd";
  survey.performNext();
  assert.equal(survey.currentSingleElement.name, "panel1", "current question, #2");
  returnResultFunc(false);
  assert.equal(survey.currentSingleElement.name, "panel1", "current question, #3");
  q1.value = "abc";
  survey.performNext();
  assert.equal(survey.currentSingleElement.name, "panel1", "current question, #4");
  returnResultFunc(true);
  assert.equal(survey.currentSingleElement.name, "q2", "current question, #5");
