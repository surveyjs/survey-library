...
import { createBoxShadow, parseBoxShadow } from "../src/utils/shadow-effects";
...
test("utils: keytoclick - skip UP if there was no DOWN", () => {
  var clicked = false;
  var event = {
    keyCode: 13,
...
test("utils: keytoclick - support tab keyup", () => {
  var classAdded = "";
  var event = {
    keyCode: 9,
    target: {
...
test("utils: devices: detectMouseSupport", () => {
  let result;
...
test("Test animation utils: enter animation", () => {
...
test("Test animation property: array", () => {
  const oldRequestAnimationFrame = window.requestAnimationFrame;
  window.requestAnimationFrame = ((cb) => cb()) as any;
  let elements = [0, 3];
...
test("Check onNextRender and cancel", () => {
  const oldRequestAnimationFrame = window.requestAnimationFrame;
...
test("test onNextRender function", () => {
...
test("check animation when rerendered event fired with isCanceled option", () => {
...
test("test getRootNode function", () => {
...
test("test getActiveElement function", () => {
  const div = document.createElement("div");
  const shadowDiv = document.createElement("div");
...
test("Check createBoxShadow and parseBoxShadow functions", () => {
  ...
  boxShadow = "var(--sjs2-border-offset-x-surface-default) var(--sjs2-border-offset-y-surface-default) var(--sjs2-border-blur-surface-default) var(--sjs2-border-spread-surface-default) var(--sjs2-color-utility-shadow-surface-default)";
  parsedBoxShadow = parseBoxShadow(boxShadow);
  expect(parsedBoxShadow[0]["color"]).toBe("var(--sjs2-color-utility-shadow-surface-default)");
