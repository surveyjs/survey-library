...
// `postId` is left empty - `renderedId` then equals the raw id (today's behavior). Captured once at
// module scope so the branch inside the component is stable across renders (rules-of-hooks safe).
...
// and CSS identifiers. We use `postId` as a suffix on ids that ARE referenced by selectors/aria, so
// strip everything but word chars and dashes. The transform is deterministic, so it stays stable
// across the SSR -> hydration boundary. A leading letter is prepended so the token never starts with
// a digit after stripping.
function sanitizePostId(token: string): string {
  const cleaned = token.replace(/[^\w-]/g, "");
  return cleaned ? "_" + cleaned : "";
}
// Assigns the framework's SSR-stable token to `survey.postId` before descendants render. Rendered as
// the first child of the Survey root so it runs before any page/question reads `renderedId`. Renders
// nothing. Inert on React 17 (no `useId`); there `postId` stays empty and single-survey SSR still
// hydrates cleanly (see v3-element-ids-02).
export function SurveyPostIdInitializer(props: { survey: SurveyModel }): null {
  if (reactUseId) {
    ...
    const pid = sanitizePostId(reactUseId());
    // idPrefix (Creator) takes precedence over postId, so only assign when postId would be used.
    if (!props.survey.idPrefix && props.survey.postId !== pid) {
      props.survey.postId = pid;
    }
  }
  return null;
}
  ...
  doRender(): React.JSX.Element {
    ...
    return (
      <div id={this.rootNodeId} ref={this.rootRef} className={cssClasses} style={this.survey.themeVariables} lang={this.survey.locale || "en"} dir={this.survey.localeDir}>
        <SurveyPostIdInitializer survey={this.survey} />
