// import { test, expect } from "@playwright/test";
// import { url, frameworks, initSurvey } from "../../visualRegressionTests/helper";

// const title = "Multipletext Screenshot";

// frameworks.forEach(framework => {
//   test.describe(`${framework} ${title}`, () => {
//     test.beforeEach(async ({ page }) => {
//       await page.goto(`${url}${framework}`);
//     });

//     test("Check multipletext question", async ({ page }) => {
//       await page.setViewportSize({ width: 1920, height: 1080 });
//       await initSurvey(framework, {
//         showQuestionNumbers: "on",
//         width: "900px",
//         questions: [
//           {
//             type: "multipletext",
//             name: "q1",
//             minWidth: "768px",
//             maxWidth: "768px",
//             width: "768px",
//             title: "Personal Information",
//             items: [
//               {
//                 name: "item1",
//                 title: "Full Name"
//               },
//               {
//                 name: "item2",
//                 title: "Email Address"
//               },
//               {
//                 name: "item3",
//                 title: "ID"
//               },
//             ]
//           },
//         ]
//       });

//       const questionRoot = page.locator(".sd-question");
//       await expect(questionRoot).toHaveScreenshot("mutlipletext.png");
//       await questionRoot.locator("input.sd-input").click();
//       await expect(questionRoot).toHaveScreenshot("mutlipletext-focus.png");
//     });

//     test("Check multipletext question error", async ({ page }) => {
//       await page.setViewportSize({ width: 1920, height: 1080 });
//       await initSurvey(framework, {
//         showQuestionNumbers: "on",
//         widthMode: "responsive",
//         questions: [
//           {
//             type: "multipletext",
//             name: "q1",
//             minWidth: "1000px",
//             maxWidth: "1000px",
//             width: "1000px",
//             colCount: 2,
//             title: "Personal Information",
//             items: [
//               {
//                 name: "item1",
//                 isRequired: true,
//                 title: "Full Name"
//               },
//               {
//                 name: "item2",
//                 title: "Email Address"
//               },
//               {
//                 name: "item3",
//                 isRequired: true,
//                 title: "ID"
//               },
//             ]
//           },
//         ]
//       });

//       const questionRoot = page.locator(".sd-question");
//       await page.click(".sd-navigation__complete-btn");
//       await page.evaluate(() => document.body.focus());
//       await expect(questionRoot).toHaveScreenshot("mutlipletext-error-top.png");
//       await page.evaluate(() => { (window as any).survey.getAllQuestions()[0].itemErrorLocation = "bottom"; });
//       await page.evaluate(() => document.body.focus());
//       await expect(questionRoot).toHaveScreenshot("mutlipletext-error-bottom.png");
//     });

//     test("Check multipletext itemTitleWidth", async ({ page }) => {
//       await page.setViewportSize({ width: 1920, height: 1080 });
//       await initSurvey(framework, {
//         showQuestionNumbers: "on",
//         width: "900px",
//         questions: [
//           {
//             type: "multipletext",
//             name: "q1",
//             itemTitleWidth: "400px",
//             items: [
//               {
//                 name: "item1",
//                 title: "A very long first item title"
//               },
//               {
//                 name: "item2",
//                 title: "A medium long title"
//               },
//               {
//                 name: "item3",
//                 title: "Short title"
//               },
//             ]
//           },
//         ]
//       });

//       const questionRoot = page.locator(".sd-question");
//       await expect(questionRoot).toHaveScreenshot("mutlipletext-titlewidth.png");
//     });
//   });
// });