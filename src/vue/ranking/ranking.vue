<template>
  <div :class="question.rootClass">
    <survey-ranking-item v-if="!question.selectToRankEnabled" v-for="(item, index) in (question.renderedRankingChoices)"
    ...
    <div v-if="question.selectToRankEnabled" :class='question.getContainerClasses("from")' data-ranking="from-container">
      <survey-ranking-item v-for="(item, index) in (question.renderedUnRankingChoices)" :key="item.value + '-' + index + '-item'"
      ...
      <div v-if="question.renderedUnRankingChoices.length === 0" :class="question.cssClasses.containerPlaceholder">
    ...
    <div v-if="question.selectToRankEnabled" :class='question.getContainerClasses("to")' data-ranking="to-container">
      <survey-ranking-item v-for="(item, index) in (question.renderedRankingChoices)" :key="item.value + '-' + index + '-item'"
        :class="question.getItemClass(item)" :text="item.locText" :index="index" :indexText="getNumberByIndex(index)"
        :cssClasses="question.cssClasses" :question="question" :item="item"></survey-ranking-item>

      <div v-if="question.renderedRankingChoices.length === 0" :class="question.cssClasses.containerPlaceholder">
