...
frameworks.forEach((framework) => {
  ...
  test("Check survey width", async (t) => {
    await t
      .expect(Selector(".sd-body").getStyleProperty("max-width")).eql("455px");
  });

  test("Check question width", async (t) => {
    await t
      .expect(Selector(".sd-row > div").getStyleProperty("min-width")).eql("min(100%, 300px)");

    await ClientFunction(() => { window["survey"].getAllQuestions()[0].minWidth = "200px"; })();
    await t
      .expect(Selector(".sd-row > div").getStyleProperty("min-width")).eql("min(100%, 200px)");
  });
});