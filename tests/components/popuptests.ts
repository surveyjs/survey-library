...
QUnit.test("autoscroll in ios", (assert) => {
  ...
  (<any>window).innerHeight = 400;
  assert.equal(viewModel.isVisible, false);
  assert.equal(viewModel["isAutoScroll"], true);
  ...
  (<any>window).scroll(0, 10);
  setTimeout(function () {
    assert.equal(viewModel.isVisible, true);
    assert.equal(viewModel["isAutoScroll"], false);
    (<any>window).scroll(0, -100);
    done();
