...
QUnit.test("Clear the isGhostRow marker", function (assert) {
  ...
  assert.equal(renderedTable.rows[0].isGhostRow, false, "Row 0 isGhostRow marker is clear initially");
  dd.dragInit(<any>{}, renderedTable.rows[0].row, matrixQuestion);
  assert.equal(renderedTable.rows[0].isGhostRow, true, "Row 0 marked as being dragged");
  dd.clear();
  assert.equal(renderedTable.rows[0].isGhostRow, false, "Row 0 isGhostRow marker is reset");
});
