...
QUnit.test("Parse item conditions", function(assert) {
    ...
    //greaterorequal
    assert.ok(
        checkItemCondition(">=", "greaterorequal"),
        ">= is greaterorequal"
    );
    assert.ok(
        checkItemCondition("=>", "greaterorequal"),
    ...
    assert.ok(
        checkItemCondition(" contain ", "contains"),
        "contain is contains"
    );
...
QUnit.test("Run age function", function(assert) {
    ...
    assert.equal(
        runner.run(values),
        false,
        "false, the person is 10 years old"
    );
});
...
QUnit.test("ConditionRunner: sumInArray", function(assert) {
    var runner = new ConditionRunner("sumInArray({a}, 'val1')");
...
QUnit.test("contain and noncontain for null arrays", function(assert) {
    ...
    assert.equal(
        runner.run(values),
        true,
        "underfined doesn't contain 1 - true"
    );
});
