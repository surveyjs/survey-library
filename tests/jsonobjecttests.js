...
(function (dxSurvey) {
    ...
    (function (JsonSerializationTests) {
        ...
        dxSurvey.JsonObject.metaData.addClass("dealer", ["name", "dummyname", "cars", "stringArray", "defaultValue", "car", "truck", "trucks", "changeNameOnSet"]);
        dxSurvey.JsonObject.metaData.setPropertyValues("dealer", "defaultValue", null, "default");
        dxSurvey.JsonObject.metaData.setPropertyValues("dealer", "changeNameOnSet", null, null, null, function (obj, value) {
            obj.name = value;
        });
        dxSurvey.JsonObject.metaData.setPropertyValues("dealer", "truck", "truck");
        dxSurvey.JsonObject.metaData.setPropertyValues("dealer", "trucks", "truck");
        dxSurvey.JsonObject.metaData.addClass("car", ["name"]);
        dxSurvey.JsonObject.metaData.addClass("truck", ["maxWeight"], function () { return new Truck(); }, "car");
        dxSurvey.JsonObject.metaData.addClass("sport", ["maxSpeed"], null, "car");
        dxSurvey.JsonObject.metaData.setCreator("sport", function () { return new SportCar(); });
        dxSurvey.JsonObject.metaData.addClass("itemvaluelistowner", ["items"]);
        dxSurvey.JsonObject.metaData.setPropertyValues("itemvaluelistowner", "items", null, null, function (obj) { return dxSurvey.ItemValue.getData(obj.items); }, function (obj, value) { dxSurvey.ItemValue.setData(obj.items, value); });
        QUnit.module("JsonSerializationTests");
        QUnit.test("Metadata for non inherited class", function (assert) {
            assert.equal(dxSurvey.JsonObject.metaData.getProperties("dealer").length, 9, "Flat properties list");
            assert.equal(dxSurvey.JsonObject.metaData.getProperties("dealer")[0].name, "name", "Name property");
        });
        QUnit.test("Metadata add at the beginning parent class properties ", function (assert) {
            assert.equal(dxSurvey.JsonObject.metaData.getProperties("truck").length, 2, "1 + 1 parent propreties");
            assert.equal(dxSurvey.JsonObject.metaData.getProperties("truck")[0].name, "name", "parent properties first");
        });
        ...
        QUnit.test("ItemValueListOwner serialization", function (assert) {
            ...
            list.items.push(new dxSurvey.ItemValue(7, "Item 1"));
            list.items.push(new dxSurvey.ItemValue(5));
            list.items.push(new dxSurvey.ItemValue("item"));
    ...
    })(JsonSerializationTests = dxSurvey.JsonSerializationTests || (dxSurvey.JsonSerializationTests = {}));
})(dxSurvey || (dxSurvey = {}));
