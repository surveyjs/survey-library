...
QUnit.test("dropdown property editor", function (assert) {
    var question = new QuestionTextModel("q1");
    var propertyGrid = new PropertyGridModel(question);
    var titleLocationQuestion = propertyGrid.survey.getQuestionByName("titleLocation");
    assert.equal(titleLocationQuestion.getType(), "dropdown", "correct property editor is created");
    assert.equal(titleLocationQuestion.choices.length, 5, "There are five choices");
    assert.equal(titleLocationQuestion.value, "default", "the value is correct");
    question.titleLocation = "top";
    assert.equal(titleLocationQuestion.value, "top", "property editor react on value chage");
    titleLocationQuestion.value = "bottom";
    assert.equal(question.titleLocation, "bottom", "property editor change the question property");
});
...
QUnit.test("column[] property editor", function (assert) {
    ...
    var propertyGrid = new PropertyGridModel(question);
    var columnsQuestion = <QuestionMatrixDynamicModel>propertyGrid.survey.getQuestionByName("columns");
    ...
    assert.equal(columnsQuestion.columns[0].cellType, "dropdown", "Correct cell type created for cellType property");
    assert.equal(columnsQuestion.visibleRows.length, 3, "There are three elements");
    assert.equal(columnsQuestion.visibleRows[0].cells[0].value, "default", "the first cell value is 'default'");
    assert.equal(columnsQuestion.visibleRows[0].cells[1].value, "col1", "the second cell value is 'col1'");
    ...
    assert.equal(question.columns[3].getType(), "matrixdropdowncolumn", "Object created correctly");

    question.columns[1].title  = "Column 2";
    assert.equal(columnsQuestion.visibleRows[1].cells[2].value, "Column 2", "the third cell in second row is correct");

    question.columns[2].cellType = "text";
    assert.equal(columnsQuestion.visibleRows[2].cells[0].value, "text", "react on changing column cell type");
    columnsQuestion.visibleRows[2].cells[0].value = "checkbox";
    assert.equal(question.columns[2].cellType, "checkbox", "change column cell type in matrix");
});
